package WebGUI::i18n::Polish::Asset_Photo;
use utf8;
our $I18N = {
  'assetName' => {
    'lastUpdated' => '1227966423',
    'message' => "Zdj\x{119}cie"
  },
  'comment message' => {
    'lastUpdated' => '1237561151',
    'message' => "Komentarz zosta\x{142} dodany. <a href=\"%s\">Powr\x{f3}t do zdj\x{119}cia</a>."
  },
  'commentForm error no bodyText' => {
    'lastUpdated' => '1237561159',
    'message' => "Nie wprowadzono \x{17c}adnego tekstu. Wprowad\x{17a} tekst aby doda\x{107} komentarz."
  },
  'commentForm error no commentId' => {
    'lastUpdated' => '1237556248',
    'message' => "Nie zosta\x{142} nadany ID komentarza. To wskazuje na problem z szablonem. Prosz\x{119} powiadomi\x{107} administratora."
  },
  'delete message' => {
    'lastUpdated' => '1227969331',
    'message' => "Zdj\x{119}cie zosta\x{142}o usuni\x{119}te.  <a href=\"%s\">Powr\x{f3}t do albumu</a>."
  },
  'editCommentSave message' => {
    'lastUpdated' => '1227969342',
    'message' => "Komentarz zosta\x{142} zaktualizowany.  <a href=\"%s\">Powr\x{f3}t do zdj\x{119}cia</a>."
  },
  'editForm cancel' => {
    'lastUpdated' => '1227966531',
    'message' => 'Anuluj'
  },
  'editForm friendsOnly' => {
    'lastUpdated' => '1237561229',
    'message' => 'Tylko dla znajomych'
  },
  'editForm keywords' => {
    'lastUpdated' => '1227966544',
    'message' => 'Tagi'
  },
  'editForm location' => {
    'lastUpdated' => '1227969382',
    'message' => 'Lokalizacja'
  },
  'editForm photo new' => {
    'lastUpdated' => '1227966565',
    'message' => "Dodaj zdj\x{119}cie"
  },
  'editForm photo replace' => {
    'lastUpdated' => '1227969419',
    'message' => "Zmie\x{144} zdj\x{119}cie"
  },
  'editForm save' => {
    'lastUpdated' => '1227967295',
    'message' => 'Zapisz'
  },
  'editForm synopsis label' => {
    'lastUpdated' => '1237561243',
    'message' => "Podpis zdj\x{119}cia"
  },
  'editForm title label' => {
    'lastUpdated' => '1227969443',
    'message' => "Tytu\x{142}"
  },
  'error no space' => {
    'lastUpdated' => '1237561255',
    'message' => "Nie masz wystarczaj\x{105}cej ilo\x{15b}ci wolnego miejsca, \x{17c}eby przes\x{142}a\x{107} to zdj\x{119}cie."
  },
  'form comment save comment' => {
    'lastUpdated' => '1237557290',
    'message' => 'Dodaj komentarz'
  },
  'help commentForm body' => {
    'lastUpdated' => '1237812862',
    'message' => "Te zmienne tworz\x{105} formularz umo\x{17c}liwiaj\x{105}cy u\x{17c}ytkownikom dodawanie komentarzy do zdj\x{119}\x{107}."
  },
  'help commentForm title' => {
    'lastUpdated' => '1237557321',
    'message' => "Zdj\x{119}cie - formularz komentarza"
  },
  'help common body' => {
    'lastUpdated' => '1237559118',
    'message' => "Te zmienne s\x{105} udost\x{119}pnione we wszystkich widokach zdj\x{119}cia."
  },
  'help common title' => {
    'lastUpdated' => '1237556945',
    'message' => "Zdj\x{119}cie - zmienne wsp\x{f3}lne"
  },
  'help delete body' => {
    'lastUpdated' => '1237557021',
    'message' => "Potwierd\x{17a} usuni\x{119}cie zdj\x{119}cia"
  },
  'help delete title' => {
    'lastUpdated' => '1237557363',
    'message' => "Zdj\x{119}cie - formularz usuwania"
  },
  'help edit body' => {
    'lastUpdated' => '1237558986',
    'message' => "Te zmienne tworz\x{105} formularz dodawania / edycji zdj\x{119}cia."
  },
  'help edit title' => {
    'lastUpdated' => '1237812837',
    'message' => "Zdj\x{119}cie - formularz edycji"
  },
  'help editComment body' => {
    'lastUpdated' => '1237558997',
    'message' => "Te zmienne s\x{105} dost\x{119}pne w szablonie edycji komentarza"
  },
  'help editComment title' => {
    'lastUpdated' => '1237557112',
    'message' => "Zdj\x{119}cie - szablon edycji komentarza"
  },
  'help makeShortcut body' => {
    'lastUpdated' => '1237559143',
    'message' => "Te zmienne tworz\x{105} formularz publikacji zdj\x{119}\x{107} w innym albumie."
  },
  'help makeShortcut title' => {
    'lastUpdated' => '1237559162',
    'message' => "Zdj\x{119}cie - formularz publikacji"
  },
  'help view body' => {
    'lastUpdated' => '1237559193',
    'message' => "Te zmienne tworz\x{105} normalny widok zdj\x{119}cia"
  },
  'help view title' => {
    'lastUpdated' => '1237559206',
    'message' => "Zdj\x{119}cie - normalny widok"
  },
  'helpvar album_menuTitle' => {
    'lastUpdated' => '1238072580',
    'message' => "Tytu\x{142} w menu albumu zawieraj\x{105}cego zdj\x{119}cie"
  },
  'helpvar album_thumbnailUrl' => {
    'lastUpdated' => '1238072590',
    'message' => "Adres URL miniatury albumu zawieraj\x{105}cego zdj\x{119}cie"
  },
  'helpvar album_title' => {
    'lastUpdated' => '1238072598',
    'message' => "Tytu\x{142} albumu zawieraj\x{105}cego zdj\x{119}cie"
  },
  'helpvar album_url' => {
    'lastUpdated' => '1238072604',
    'message' => "Adres URL albumu zawieraj\x{105}cego zdj\x{119}cie"
  },
  'helpvar canComment' => {
    'lastUpdated' => '1237559407',
    'message' => "Warunek spe\x{142}niony gdy aktualny u\x{17c}ytkownik mo\x{17c}e komentowa\x{107} zdj\x{119}cie"
  },
  'helpvar canEdit' => {
    'lastUpdated' => '1237557179',
    'message' => "Warunek spe\x{142}niony je\x{17c}eli aktualny u\x{17c}ytkownik mo\x{17c}e edytowa\x{107} zdj\x{119}cie."
  },
  'helpvar commentForm_bodyText' => {
    'lastUpdated' => '1238072635',
    'message' => "Pole na tre\x{15b}\x{107} komentarza. Edytor skonfigurowany przez nadrz\x{119}dn\x{105} galeri\x{119}."
  },
  'helpvar commentForm_end' => {
    'lastUpdated' => '1238072642',
    'message' => 'Koniec formularza komentarzy'
  },
  'helpvar commentForm_start' => {
    'lastUpdated' => '1238072647',
    'message' => "Pocz\x{105}tek formularza komentarza"
  },
  'helpvar commentForm_submit' => {
    'lastUpdated' => '1237985212',
    'message' => "Zmienna zwracaj\x{105}ca przycisk wysy\x{142}aj\x{105}cy formularz komentarza"
  },
  'helpvar commentLoop' => {
    'lastUpdated' => '1237985278',
    'message' => "P\x{119}tla po komentarzach dla zdj\x{119}cia"
  },
  'helpvar commentLoop bodyText' => {
    'lastUpdated' => '1238072656',
    'message' => "Tre\x{15b}\x{107} komentarza"
  },
  'helpvar commentLoop creationDate' => {
    'lastUpdated' => '1238072666',
    'message' => 'Data dodania komentarza'
  },
  'helpvar commentLoop url_deleteComment' => {
    'lastUpdated' => '1238072672',
    'message' => "Adres URL usuwaj\x{105}cy komentarz."
  },
  'helpvar commentLoop userId' => {
    'lastUpdated' => '1238072685',
    'message' => "ID u\x{17c}ytkownika kt\x{f3}ry doda\x{142} komentarz."
  },
  'helpvar commentLoop username' => {
    'lastUpdated' => '1238072699',
    'message' => "Nazwa u\x{17c}ytkownika, kt\x{f3}ry doda\x{142} komentarz."
  },
  'helpvar commentLoop visitorIp' => {
    'lastUpdated' => '1238072720',
    'message' => "IP u\x{17c}ytkownika je\x{17c}eli u\x{17c}ytkownik jest go\x{15b}ciem."
  },
  'helpvar commentLoop_pageBar' => {
    'lastUpdated' => '1237985384',
    'message' => "Zmienna zwracaj\x{105}ca pasek do nawigacji po stronach komentarzy"
  },
  'helpvar error' => {
    'lastUpdated' => '1237985418',
    'message' => "Ten i18n komunikat b\x{142}\x{119}du"
  },
  'helpvar errors' => {
    'lastUpdated' => '1237985469',
    'message' => "P\x{119}tla zwracaj\x{105}ca komunikaty b\x{142}\x{119}du do pokazania u\x{17c}ytkownikowi."
  },
  'helpvar exifLoop' => {
    'lastUpdated' => '1238073593',
    'message' => "P\x{119}tla znacznik\x{f3}w EXIF"
  },
  'helpvar exifLoop tag' => {
    'lastUpdated' => '1238073605',
    'message' => 'Nazwa znacznika EXIF'
  },
  'helpvar exifLoop value' => {
    'lastUpdated' => '1238073615',
    'message' => "Warto\x{15b}\x{107} znacznika EXIF"
  },
  'helpvar exif_' => {
    'lastUpdated' => '1238567020',
    'message' => "Ka\x{17c}dy tag EXIF mo\x{17c}e by\x{107} zwi\x{105}zany z nazw\x{105}."
  },
  'helpvar fileUrl' => {
    'lastUpdated' => '1238072745',
    'message' => "Adres URL zdj\x{119}cia w oryginalnych rozmiarach."
  },
  'helpvar form_end' => {
    'lastUpdated' => '1238072751',
    'message' => 'Koniec formularza.'
  },
  'helpvar form_friendsOnly' => {
    'lastUpdated' => '1238073278',
    'message' => "Ustanowi\x{107} zdj\x{119}ciem tylko dla znajomych?"
  },
  'helpvar form_keywords' => {
    'lastUpdated' => '1238072757',
    'message' => "Pole formularza do dodawania s\x{142}\x{f3}w kluczowych dla zdj\x{119}cia."
  },
  'helpvar form_location' => {
    'lastUpdated' => '1238568152',
    'message' => "Lokalizacja zdj\x{119}cia zosta\x{142}a pobrana."
  },
  'helpvar form_parentId' => {
    'lastUpdated' => '1237985838',
    'message' => "Wybierz album, kt\x{f3}rego skr\x{f3}t powinien by\x{107} stworzony."
  },
  'helpvar form_photo' => {
    'lastUpdated' => '1237985578',
    'message' => "Zdj\x{119}cie do za\x{142}adownia."
  },
  'helpvar form_start' => {
    'lastUpdated' => '1238072764',
    'message' => "Pocz\x{105}tek formularza."
  },
  'helpvar form_submit' => {
    'lastUpdated' => '1238072771',
    'message' => "Przycisk wysy\x{142}aj\x{105}cy formularz."
  },
  'helpvar form_synopsis' => {
    'lastUpdated' => '1238568180',
    'message' => "Podpis do zdj\x{119}cia."
  },
  'helpvar form_title' => {
    'lastUpdated' => '1238072778',
    'message' => "Tytu\x{142} zdj\x{119}cia."
  },
  'helpvar gallery_menuTitle' => {
    'lastUpdated' => '1238072820',
    'message' => "Tytu\x{142} wy\x{15b}wietlany w menu galerii zawieraj\x{105}cej zdj\x{119}cie."
  },
  'helpvar gallery_title' => {
    'lastUpdated' => '1238072826',
    'message' => "Tytu\x{142} galerii zawieraj\x{105}cej zdj\x{119}cie."
  },
  'helpvar gallery_url' => {
    'lastUpdated' => '1238072833',
    'message' => "Adres URL galerii zawieraj\x{105}cej zdj\x{119}cie."
  },
  'helpvar isNewPhoto' => {
    'lastUpdated' => '1237560422',
    'message' => "Warunek spe\x{142}niony je\x{17c}eli u\x{17c}ytkownik dodaje nowe zdj\x{119}cie"
  },
  'helpvar isPending' => {
    'lastUpdated' => '1238072846',
    'message' => "Flaga do ustawienia czy zdj\x{119}cie nie jest jeszcze zatwierdzone. U\x{17c}ytkownicy kt\x{f3}rzy mog\x{105} edytowa\x{107} zdj\x{119}cie mog\x{105} je ogl\x{105}dn\x{105}\x{107} zanim je zatwierdz\x{105}."
  },
  'helpvar keyword' => {
    'lastUpdated' => '1238073729',
    'message' => "S\x{142}owo kluczowe"
  },
  'helpvar keywords' => {
    'lastUpdated' => '1238073763',
    'message' => "P\x{119}tla po s\x{142}owach kluczowych skojarzonych z tym zdj\x{119}ciem"
  },
  'helpvar numberOfComments' => {
    'lastUpdated' => '1238072854',
    'message' => "Ilo\x{15b}\x{107} komentarzy dla zdj\x{119}cia"
  },
  'helpvar ownerUsername' => {
    'lastUpdated' => '1238072863',
    'message' => "Nazwa u\x{17c}ytkownika kt\x{f3}ry doda\x{142} zdj\x{119}cie"
  },
  'helpvar resolutions_' => {
    'lastUpdated' => '1238568292',
    'message' => "Adres URL przekierowuj\x{105}cy do znanej rozdzielczo\x{15b}ci. Rozdzielczo\x{15b}\x{107} &quot;800&quot; mo\x{17c}na by zapisa\x{107} jako adres URL &quot;rozdzielczosc_800&quot;"
  },
  'helpvar resolutions_loop' => {
    'lastUpdated' => '1238072894',
    'message' => "P\x{119}tla zawieraj\x{105}ca list\x{119} dost\x{119}pnych rozdzielczo\x{15b}ci zdj\x{119}cia."
  },
  'helpvar resolutions_loop resolution' => {
    'lastUpdated' => '1238072901',
    'message' => "Rozdzielczo\x{15b}\x{107} zdj\x{119}cia."
  },
  'helpvar resolutions_loop url_download' => {
    'lastUpdated' => '1238072908',
    'message' => "Adres URL danej rozdzielczo\x{15b}ci zdj\x{119}cia do pobrania."
  },
  'helpvar synopsis_text' => {
    'lastUpdated' => '1238568396',
    'message' => "Pole &quot;Streszczenie&quot; z usuni\x{119}tymi znacznikami HTML"
  },
  'helpvar thumbnailUrl' => {
    'lastUpdated' => '1238072919',
    'message' => "Adres URL do miniaturki zdj\x{119}cia"
  },
  'helpvar url_addArchive' => {
    'lastUpdated' => '1238072925',
    'message' => 'Adres URL do dodawania archiwów do albumu.'
  },
  'helpvar url_album' => {
    'lastUpdated' => '1238072931',
    'message' => "Adres URL do albumu zawieraj\x{105}cego zdj\x{119}cie."
  },
  'helpvar url_delete' => {
    'lastUpdated' => '1238072937',
    'message' => "Adres URL do usuni\x{119}cia zdj\x{119}cia."
  },
  'helpvar url_demote' => {
    'lastUpdated' => '1238072942',
    'message' => "Adres URL degraduj\x{105}cy zdj\x{119}cie w rankingu. "
  },
  'helpvar url_edit' => {
    'lastUpdated' => '1238072948',
    'message' => "Adres URL do edycji zdj\x{119}cia"
  },
  'helpvar url_gallery' => {
    'lastUpdated' => '1238072954',
    'message' => "Adres URL do galerii zawieraj\x{105}cej zdj\x{119}cie."
  },
  'helpvar url_listFilesForOwner' => {
    'lastUpdated' => '1238072961',
    'message' => "Adres URL do listy zdj\x{119}\x{107} i album\x{f3}w dodanych przez w\x{142}a\x{15b}ciciela zdj\x{119}cia."
  },
  'helpvar url_makeShortcut' => {
    'lastUpdated' => '1238072972',
    'message' => "Adres URL do stworzenia skr\x{f3}tu dla zdj\x{119}cia."
  },
  'helpvar url_promote' => {
    'lastUpdated' => '1238072978',
    'message' => "Adres URL linku podnosz\x{105}cego zdj\x{119}cie w rankingu."
  },
  'helpvar url_searchKeyword' => {
    'lastUpdated' => '1238073002',
    'message' => "Adres URL strony wyszukiwania po s\x{142}owie kluczowym"
  },
  'helpvar url_searchKeywordUser' => {
    'lastUpdated' => '1238073010',
    'message' => "Adres URL do strony wyszukiwania po s\x{142}owie kluczowym. Wyszukiwanie odbywa si\x{119} jedynie po zdj\x{119}ciach dodanych przez aktualnie zalogowanego u\x{17c}ytkownika."
  },
  'helpvar url_slideshow' => {
    'lastUpdated' => '1238073017',
    'message' => "Adres URL do widoku slajd\x{f3}w w albumie zawieraj\x{105}cym zdj\x{119}cie."
  },
  'helpvar url_thumbnails' => {
    'lastUpdated' => '1238073024',
    'message' => "Adres URL miniaturki albumu zawieraj\x{105}cego zdj\x{119}cie"
  },
  'helpvar url_yes' => {
    'lastUpdated' => '1237557957',
    'message' => "Potwierd\x{17a} usuwanie tego zdj\x{119}cia."
  },
  'hide' => {
    'lastUpdated' => '1237556652',
    'message' => 'Ukryj'
  },
  'more details' => {
    'lastUpdated' => '1237557397',
    'message' => "Wi\x{119}cej szczeg\x{f3}\x{142}\x{f3}w "
  },
  'save message' => {
    'lastUpdated' => '1237557939',
    'message' => "Twoje zdj\x{119}cie zosta\x{142}o wys\x{142}ane do zatwierdzenia. <a href=\"%s\">Zobacz zdj\x{119}cie</a>. <a href=\"%s\">Dodaj kolejne zdj\x{119}cie</a>."
  },
  'template assetName' => {
    'lastUpdated' => '1227967371',
    'message' => "Zdj\x{119}cie"
  },
  'template comment add title' => {
    'lastUpdated' => '1227967414',
    'message' => 'Dodaj komentarz'
  },
  'template comment creationDate' => {
    'lastUpdated' => '1227967456',
    'message' => 'Dodany'
  },
  'template comment delete confirm' => {
    'lastUpdated' => '1227967536',
    'message' => "Jeste\x{15b} pewny, \x{17c}e chcesz usun\x{105}\x{107} ten komentarz?"
  },
  'template comment edit comment' => {
    'lastUpdated' => '1237557440',
    'message' => 'Edytuj komentarz'
  },
  'template comment edit title' => {
    'lastUpdated' => '1227967524',
    'message' => 'Edytuj komentarz'
  },
  'template comments title' => {
    'lastUpdated' => '1227967516',
    'message' => 'Komentarze'
  },
  'template creationDate' => {
    'lastUpdated' => '1237557572',
    'message' => 'Dodany'
  },
  'template delete albums' => {
    'lastUpdated' => '1237557459',
    'message' => "Zdj\x{119}cie jest obecne w tych albumach: "
  },
  'template delete message' => {
    'lastUpdated' => '1237557620',
    'message' => "Czy jeste\x{15b} pewien, \x{17c}e chcesz to usun\x{105}\x{107}?"
  },
  'template error happened' => {
    'lastUpdated' => '1237823692',
    'message' => "Wyst\x{105}pi\x{142} b\x{142}\x{105}d w trakcie przetwarzania twojego zapytania."
  },
  'template fileUrl' => {
    'lastUpdated' => '1237557586',
    'message' => "Poka\x{17c} w pe\x{142}nych rozmiarach"
  },
  'template friendsOnly label' => {
    'lastUpdated' => '1237557555',
    'message' => 'Prywatne'
  },
  'template friendsOnly no' => {
    'lastUpdated' => '1237557535',
    'message' => 'Publiczne'
  },
  'template friendsOnly yes' => {
    'lastUpdated' => '1237557522',
    'message' => 'Tylko dla znajomych'
  },
  'template keywords' => {
    'lastUpdated' => '1237557510',
    'message' => 'Tagi'
  },
  'template location' => {
    'lastUpdated' => '1237557500',
    'message' => 'Lokalizacja'
  },
  'template makeShortcut album' => {
    'lastUpdated' => '1237557725',
    'message' => 'Wybierz album'
  },
  'template makeShortcut file' => {
    'lastUpdated' => '1237557738',
    'message' => "Zdj\x{119}cie"
  },
  'template makeShortcut title' => {
    'lastUpdated' => '1237557795',
    'message' => 'Opublikuj w innym albumie'
  },
  'template url_album' => {
    'lastUpdated' => '1237557473',
    'message' => 'Powrót do albumu'
  },
  'template url_delete' => {
    'lastUpdated' => '1237556702',
    'message' => "Usu\x{144} zdj\x{119}cie"
  },
  'template url_deleteComment' => {
    'lastUpdated' => '1237556798',
    'message' => "Usu\x{144}"
  },
  'template url_edit' => {
    'lastUpdated' => '1237556721',
    'message' => "Edytuj zdj\x{119}cie"
  },
  'template url_makeShortcut' => {
    'lastUpdated' => '1237557700',
    'message' => 'Opublikuj w innym albumie'
  },
  'template view available resolutions' => {
    'lastUpdated' => '1237556861',
    'message' => "Dost\x{119}pne rozdzielczo\x{15b}ci"
  },
  'template view details' => {
    'lastUpdated' => '1237556789',
    'message' => "Szczeg\x{f3}\x{142}y"
  },
  'template view title' => {
    'lastUpdated' => '1237556780',
    'message' => "Szczeg\x{f3}\x{142}y zdj\x{119}cia"
  },
  'template views' => {
    'lastUpdated' => '1227967662',
    'message' => "Liczba wy\x{15b}wietle\x{144}"
  }
}
;

1;
