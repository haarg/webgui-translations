package WebGUI::i18n::Polish::Asset;
use utf8;
our $I18N = {
  '%s is not a valid depth' => {
    'lastUpdated' => '1236939487',
    'message' => "nieprawid\x{142}owe zagnie\x{17c}d\x{17c}enie %s"
  },
  104 => {
    'lastUpdated' => '1236939492',
    'message' => 'URL'
  },
  '104 description' => {
    'lastUpdated' => '1236940714',
    'message' => "<p>Adres URL dla zasobu/asset. Musi by\x{107} unikalny. Je\x{15b}li operator nie wprowadzi warto\x{15b}ci zostanie utworzony na bazie aktualnego adresu URL i zawarto\x{15b}ci pola tytu\x{142}u dla menu.</p>"
  },
  105 => {
    'lastUpdated' => '1203962976',
    'message' => "Poka\x{17c}"
  },
  107 => {
    'lastUpdated' => '1203962983',
    'message' => "Bezpiecze\x{144}stwo"
  },
  1073 => {
    'lastUpdated' => '1203963013',
    'message' => 'Szablon stylu'
  },
  1079 => {
    'lastUpdated' => '1236939502',
    'message' => 'Styl drukowalny'
  },
  108 => {
    'lastUpdated' => '1236939513',
    'message' => "W\x{142}a\x{15b}ciciel"
  },
  '108 description' => {
    'lastUpdated' => '1236940754',
    'message' => "<p>W\x{142}a\x{15b}cicielem zasobu/asset zwykle jest osoba, kt\x{f3}ra go utworzy\x{142}a. Ten u\x{17c}ytkownik posiada pe\x{142}ne prawa do edycji i wy\x{15b}wietlenia. </p><p>UWAGA: W\x{142}a\x{15b}ciciel mo\x{17c}e by\x{107} zmieniony wy\x{142}\x{105}cznie przez administratora.</p>"
  },
  1082 => {
    'lastUpdated' => '1203963199',
    'message' => 'Schowek'
  },
  1083 => {
    'lastUpdated' => '1236939547',
    'message' => 'Nowy kontent'
  },
  11 => {
    'lastUpdated' => '1236867850',
    'message' => "Opr\x{f3}\x{17c}nij kosz."
  },
  174 => {
    'lastUpdated' => '1203963224',
    'message' => "Pokaza\x{107} tytu\x{142}?"
  },
  411 => {
    'lastUpdated' => '1203963232',
    'message' => "Tytu\x{142} w menu"
  },
  '411 description' => {
    'lastUpdated' => '1236940764',
    'message' => "<p>Kr\x{f3}tki tytu\x{142} jaki pojawi si\x{119} w elementach nawigacyjnych serwisu WWW. Pozostawiony pusty b\x{119}dzie uzupe\x{142}niony warto\x{15b}ci\x{105} pola Tytu\x{142}.</p>"
  },
  412 => {
    'lastUpdated' => '1236939588',
    'message' => 'Podsumowanie'
  },
  '412 description' => {
    'lastUpdated' => '1236940774',
    'message' => '<p>Krótki opis tego zasobu/asset.</p>'
  },
  43 => {
    'lastUpdated' => '1236940781',
    'message' => "Czy jeste\x{15b} pewny, \x{17c}e chcesz usun\x{105}\x{107} t\x{105} zawarto\x{15b}\x{107}? Zauwa\x{17c}, \x{17c}e ta zmiana nie jest wersjonowana i odniesie skutek natychmiastowy."
  },
  486 => {
    'lastUpdated' => '1236939647',
    'message' => 'Typ danej'
  },
  487 => {
    'lastUpdated' => '1236939657',
    'message' => "Mo\x{17c}liwe warto\x{15b}ci"
  },
  85 => {
    'lastUpdated' => '1236939663',
    'message' => 'Opis'
  },
  871 => {
    'lastUpdated' => '1236939668',
    'message' => "Kto mo\x{17c}e edytowa\x{107}?"
  },
  '871 description' => {
    'lastUpdated' => '1236940793',
    'message' => "<p>Wybierz grup\x{119}, kt\x{f3}rej cz\x{142}onkowie mog\x{105} edytowa\x{107} ten zas\x{f3}b/asset. Cz\x{142}onkowie grupy mog\x{105} r\x{f3}wnie\x{17c} ogl\x{105}da\x{107} zas\x{f3}b.</p>"
  },
  872 => {
    'lastUpdated' => '1236940799',
    'message' => "Kto mo\x{17c}e ogl\x{105}da\x{107}?"
  },
  '872 description' => {
    'lastUpdated' => '1236940814',
    'message' => "<p>Wybierz grup\x{119}, kt\x{f3}rej cz\x{142}onkowie mog\x{105} ogl\x{105}da\x{107} zas\x{f3}b/asset. Je\x{15b}li chcia\x{142}by\x{15b} przydzieli\x{107} prawo ogl\x{105}dania go\x{15b}ciom i zarejestrowanym u\x{17c}ytkownikom wybierz grup\x{119} &quot;Everybody&quot;.</p>"
  },
  886 => {
    'lastUpdated' => '1236939773',
    'message' => "Ukry\x{107} w nawigacji/menu?"
  },
  '886 description' => {
    'lastUpdated' => '1236940831',
    'message' => "<p>Czy zas\x{f3}b/asset ma by\x{107} ukryty w elementach nawigacyjnych i mapie serwisu.</p>"
  },
  895 => {
    'lastUpdated' => '1236939800',
    'message' => "Przedawnienie pami\x{119}ci podr\x{119}cznej"
  },
  896 => {
    'lastUpdated' => '1236939814',
    'message' => "Przedawnienie pami\x{119}ci podr\x{119}cznej (Go\x{15b}cie)"
  },
  940 => {
    'lastUpdated' => '1203963907',
    'message' => "Otworzy\x{107} w nowym oknie?"
  },
  '940 description' => {
    'lastUpdated' => '1236940874',
    'message' => "<p>Wybierz tak aby otworzy\x{107} ten zas\x{f3}b/asset w nowym oknie. Zauwa\x{17c} jednak, \x{17c}e z nowymi oknami wi\x{105}\x{17c}e si\x{119} wiele problem\x{f3}w. Na wielu przegl\x{105}darkach okna mog\x{105} by\x{107} niedost\x{119}pne, gdy u\x{17c}ytkownik wy\x{142}\x{105}czy obs\x{142}ug\x{119} JavaScript albo zablokuje wyskakuj\x{105}ce okienka. Korzystaj z tej cechy rozwa\x{17c}nie.</p>"
  },
  949 => {
    'lastUpdated' => '1236940880',
    'message' => "Zarz\x{105}dzaj schowkiem."
  },
  950 => {
    'lastUpdated' => '1203964110',
    'message' => "Wyczy\x{15b}\x{107} schowek."
  },
  951 => {
    'lastUpdated' => '1236939877',
    'message' => "Czy jeste\x{15b} pewny, \x{17c}e zamierzasz wyczy\x{15b}ci\x{107} schowek i przerzuci\x{107} zawarto\x{15b}\x{107} do kosza?"
  },
  954 => {
    'lastUpdated' => '1236939882',
    'message' => "Zarz\x{105}dzaj schowkiem systemowym."
  },
  959 => {
    'lastUpdated' => '1236939893',
    'message' => "Wyczy\x{15b}\x{107} schowek systemowy."
  },
  964 => {
    'lastUpdated' => '1236939898',
    'message' => "Zarz\x{105}dzaj koszem systemowym."
  },
  965 => {
    'lastUpdated' => '1236939907',
    'message' => 'Kosz systemowy'
  },
  966 => {
    'lastUpdated' => '1203964194',
    'message' => 'Schowek systemowy'
  },
  99 => {
    'lastUpdated' => '1203964201',
    'message' => "Tytu\x{142}"
  },
  '99 description' => {
    'lastUpdated' => '1236940909',
    'message' => "<p>Tytu\x{142} zasobu/asset.  Powinien by\x{107} opisowy, ale nie za d\x{142}ugi. W przypadku braku warto\x{15b}ci b\x{119}dzie wype\x{142}niony tekstem &quot;Untitled&quot;. Makra, kod HTML i JavaScript nie mog\x{105} by\x{107} umieszczone w tytule.</p> \r\n<p><em>Notatka:</em> Zawsze powiniene\x{15b} wprowadzi\x{107} tytu\x{142}, nawet je\x{15b}li zas\x{f3}b/asset nie wykorzysta go w przysz\x{142}o\x{15b}ci. W wielu miejscach serwisu jak drzewo stron, mapa serwisu, schowek i kosz  <strong>Tytu\x{142}</strong> jest u\x{17c}yty do rozr\x{f3}\x{17c}nienia zasob\x{f3}w.</p>"
  },
  'Add new field' => {
    'lastUpdated' => '1236939988',
    'message' => "dodaj w\x{142}asno\x{15b}\x{107} metadata"
  },
  'Add new field description' => {
    'lastUpdated' => '1236940919',
    'message' => "<p>Otw\x{f3}rz formularz, w kt\x{f3}rym mo\x{17c}na doda\x{107} pola metadata do zasobu/asset.</p>"
  },
  'Class Icon' => {
    'lastUpdated' => '1203964579',
    'message' => 'Ikona klasy'
  },
  'Class Icon help' => {
    'lastUpdated' => '1236940933',
    'message' => "<p>Ka\x{17c}dy zas\x{f3}b/asset wy\x{15b}wietla r\x{f3}wnie\x{17c} ikon\x{119} klasy. Ikony Klas s\x{105} unikalne (w wi\x{119}kszo\x{15b}ci) i s\x{105} mniejszymi wersjami ikon wy\x{15b}wietlanych w sekcji Nowy Kontent w Konsoli Administratora. Klikni\x{119}cie na Ikon\x{119} Klasy rozwinie menu z dodatkowymi opcjami edycji.</p>"
  },
  'Copy' => {
    'lastUpdated' => '1225266934',
    'message' => 'Kopiuj'
  },
  'Data Type description' => {
    'lastUpdated' => '1236940070',
    'message' => '<p>Wybierz type dla elementu tego formularza.</p>'
  },
  'Delete Metadata field' => {
    'lastUpdated' => '1203964847',
    'message' => "Usu\x{144} w\x{142}asno\x{15b}\x{107} Metadata"
  },
  'Depth' => {
    'lastUpdated' => '1203964939',
    'message' => "G\x{142}\x{119}boko\x{15b}\x{107}"
  },
  'Depth description' => {
    'lastUpdated' => '1236940943',
    'message' => "<p>Ustala g\x{142}\x{119}boko\x{15b}\x{107} eksportu drzewa stron. U\x{17c}yj g\x{142}\x{119}boko\x{15b}ci 0 dla eksportu wy\x{142}\x{105}cznie aktualnej strony. </p>"
  },
  'Edit Metadata' => {
    'lastUpdated' => '1203965028',
    'message' => "edycja w\x{142}asno\x{15b}ci Metadata"
  },
  'Error: Cannot instantiate template' => {
    'lastUpdated' => '1236940091',
    'message' => "B\x{142}\x{105}d: Nie uda\x{142}o si\x{119} utworzy\x{107} szablonu"
  },
  'Export Page' => {
    'lastUpdated' => '1203965038',
    'message' => "Eksportuj stron\x{119}"
  },
  'Export as user' => {
    'lastUpdated' => '1203965054',
    'message' => "Eksportuj jako u\x{17c}ytkownik"
  },
  'Export as user description' => {
    'lastUpdated' => '1236940954',
    'message' => "<p>Wywo\x{142}aj eksport jako ten u\x{17c}ytkownik. Domy\x{15b}lnie Go\x{15b}\x{107}.</p>"
  },
  'Export site root URL' => {
    'lastUpdated' => '1227595264',
    'message' => 'Eksport adresu URL korzenia serwisu'
  },
  'Export site root URL description' => {
    'lastUpdated' => '1236940140',
    'message' => "adres URL do przej\x{15b}cia w Macro Widgets."
  },
  'Extras URL' => {
    'lastUpdated' => '1236940159',
    'message' => 'Adres URL ekstra plików (extras)'
  },
  'Extras URL description' => {
    'lastUpdated' => '1236940964',
    'message' => "<p>Ustala adres URL dla ekstra plik\x{f3}w (extras). Domy\x{15b}lnie przyjmuje warto\x{15b}\x{107} zdefiniowan\x{105} w konfiguracji WebGUI.</p>"
  },
  'Field Id' => {
    'lastUpdated' => '1203965257',
    'message' => 'Pole Id'
  },
  'Field Name description' => {
    'lastUpdated' => '1236940979',
    'message' => "<p>Nazwa w\x{142}asno\x{15b}ci metadata. Musi by\x{107} unikalna. Rozs\x{105}dne jest wykorzystywanie wy\x{142}\x{105}cznie liter (a-z), liczb (0-9) i znaku podkre\x{15b}lenia (_). A tak\x{17c}e mniej ni\x{17c} 100 znak\x{f3}w d\x{142}ugo\x{15b}ci.</p>"
  },
  'Field name' => {
    'lastUpdated' => '1203965394',
    'message' => 'Nazwa pola'
  },
  'Metadata' => {
    'lastUpdated' => '1203965401',
    'message' => 'Metadata'
  },
  'Metadata Description description' => {
    'lastUpdated' => '1236940991',
    'message' => "<p>Opcjonalny opis dla w\x{142}asno\x{15b}ci metadata. Tekst ten jest wy\x{15b}wietlany po najechaniu na zak\x{142}adk\x{119} w\x{142}asno\x{15b}ci.</p>"
  },
  'More' => {
    'lastUpdated' => '1227595396',
    'message' => "Wi\x{119}cej"
  },
  'Page Export' => {
    'lastUpdated' => '1203965482',
    'message' => 'Strona, Eksport'
  },
  'Page Export Status' => {
    'lastUpdated' => '1203965499',
    'message' => 'Status Eksportu Strony'
  },
  'Possible Values description' => {
    'lastUpdated' => '1236941005',
    'message' => "<p>Pole to wykorzystywane jest wy\x{142}\x{105}cznie dla typ\x{f3}w Radio List i Select List. Wprowad\x{17a} warto\x{15b}ci jakie maj\x{105} si\x{119} pokaza\x{107}. Jedna warto\x{15b}\x{107} w jednej linii.</p><p>Je\x{15b}li chcia\x{142}by\x{15b} uzyska\x{107} r\x{f3}\x{17c}ne etykiety dla poszczeg\x{f3}lnych warto\x{15b}ci istnieje mo\x{17c}liwo\x{15b}\x{107} specjalnego sformatowania listy:</p><pre>{<br />   &quot;key1&quot;=&gt;&quot;value1&quot;,<br />   &quot;key2&quot;=&gt;&quot;value2&quot;,<br />   &quot;key3&quot;=&gt;&quot;value3&quot;<br />   ...<br />}</pre><p>Pami\x{119}taj, aby poprawnie zastosowa\x{107} nawiasy, cudzys\x{142}owy, przecinki i inne wymagane znaki. Najlepiej podmie\x{144} warto\x{15b}ci wewn\x{105}trz cudzys\x{142}ow\x{f3}w na nowe pary kluczy i warto\x{15b}ci. Je\x{15b}li potrzebujesz wi\x{119}cej pozycji wystarczy powieli\x{107} linie wewn\x{105}trz klamer {}. </p>"
  },
  'Select' => {
    'lastUpdated' => '1236940284',
    'message' => 'Select...'
  },
  'Uploads URL' => {
    'lastUpdated' => '1236940294',
    'message' => 'Uploads URL'
  },
  'Uploads URL description' => {
    'lastUpdated' => '1236941031',
    'message' => "<p>Ustala adres URL dla plik\x{f3}w Uploads. Domy\x{15b}lnie warto\x{15b}\x{107} ustawiona w pliku konfiguracyjnym WebGUI.</p>"
  },
  'add the missing page' => {
    'lastUpdated' => '1203965700',
    'message' => "Dodaj brakuj\x{105}c\x{105} stron\x{119}."
  },
  'ago' => {
    'lastUpdated' => '1203965707',
    'message' => 'temu'
  },
  'apply' => {
    'lastUpdated' => '1225266873',
    'message' => 'zastosuj'
  },
  'asset' => {
    'lastUpdated' => '1236941042',
    'message' => 'Zasób/Asset'
  },
  'asset id' => {
    'lastUpdated' => '1236941051',
    'message' => 'Zasób/Asset ID'
  },
  'asset id description' => {
    'lastUpdated' => '1236941089',
    'message' => "<p>To jest unikalny identyfikator w WebGUI u\x{17c}ywany do \x{15b}ledzenia instancji Zasobu/Asset. Zwykli u\x{17c}ytkownicy nie musz\x{105} si\x{119} martwi\x{107} o ID Zasobu/Asset, jednak czasami zaawansowani u\x{17c}ytkownicy mog\x{105} potrzebowa\x{107} zna\x{107} go dla r\x{f3}\x{17c}nych rzeczy jak np. Raporty SQL. Nie mo\x{17c}na modyfikowa\x{107} ID Zasobu/Asset.</p>"
  },
  'asset list title' => {
    'lastUpdated' => '1236941098',
    'message' => "Zas\x{f3}b/Asset, Lista dost\x{119}pu"
  },
  'asset template asset var title' => {
    'lastUpdated' => '1236941111',
    'message' => 'Szablon Zasobu/Asset Zmienne Zasobu/Asset'
  },
  'asset template title' => {
    'lastUpdated' => '1236941119',
    'message' => 'Zmienne Szablonu Zasobu/Asset'
  },
  'assetSize' => {
    'lastUpdated' => '1236941143',
    'message' => "Jak du\x{17c}y w bajtach jest ten zas\x{f3}b/asset.  Suma wszystkich p\x{f3}l bazy danych i za\x{142}\x{105}cznik\x{f3}w."
  },
  'assets' => {
    'lastUpdated' => '1236941152',
    'message' => 'Zasoby/Assets'
  },
  'bad user privileges' => {
    'lastUpdated' => '1236941158',
    'message' => "U\x{17c}ytkownik nie posiada uprawnie\x{144} dla wy\x{15b}wietlenia tej strony."
  },
  'change' => {
    'lastUpdated' => '1236941166',
    'message' => "Zmieni\x{107} rekurencyjnie?"
  },
  'change url' => {
    'lastUpdated' => '1204058676',
    'message' => 'Zmiana URL'
  },
  'change url help' => {
    'lastUpdated' => '1236941178',
    'message' => 'Aktywowanie zmiany URL dla tego zasobu/asset.'
  },
  'committed versions' => {
    'lastUpdated' => '1204058739',
    'message' => 'Wersje Zatwierdzone'
  },
  'confirm change' => {
    'lastUpdated' => '1204058798',
    'message' => "Jete\x{15b} pewny?"
  },
  'confirm change url message' => {
    'lastUpdated' => '1236941194',
    'message' => "Ustawienie na 'Tak' potwierdzi, \x{17c}e zamierzasz permanentnie zmieni\x{107} ten adres URL, a co za tym idzie usun\x{105}\x{107} wszystkie stare rewizje tego zasobu."
  },
  'content profiling' => {
    'lastUpdated' => '1236941307',
    'message' => "Dostosowanie Zawarto\x{15b}ci"
  },
  'controls' => {
    'lastUpdated' => '1236941328',
    'message' => "To s\x{105} ikony i adresy URL pozwalaj\x{105}ce edytowa\x{107}, wycina\x{107}, kopiowa\x{107}, us\x{f3}wa\x{107} i zmienia\x{107} kolejno\x{15b}\x{107} Zasob\x{f3}w/Assets."
  },
  'copy' => {
    'lastUpdated' => '1227594751',
    'message' => 'kopiuj'
  },
  'could not create' => {
    'lastUpdated' => '1236941338',
    'message' => "Nie mo\x{17c}na utworzy\x{107} %s: %s"
  },
  'could not create path' => {
    'lastUpdated' => '1236941371',
    'message' => "Nie mo\x{17c}na utworzy\x{107} %-s poniewa\x{17c} %-s <br /> Oznacza to prawdopodobnie, \x{17c}e posiadasz stron\x{119} z t\x{105} sam\x{105} nazw\x{105} co folder kt\x{f3}ry pr\x{f3}bujesz utworzy\x{107}.<br /> "
  },
  'could not open path' => {
    'lastUpdated' => '1236941378',
    'message' => "Nie mo\x{17c}na otworzy\x{107} %-s poniewa\x{17c} %-s <br /> "
  },
  'could not rmdir' => {
    'lastUpdated' => '1204059300',
    'message' => "Nie mo\x{17c}na usun\x{105}\x{107} katalogu %s: %s"
  },
  'could not symlink' => {
    'lastUpdated' => '1236941386',
    'message' => "Nie mo\x{17c}na podlinkowa\x{107} %s do %s: %s"
  },
  'could not unlink' => {
    'lastUpdated' => '1204059363',
    'message' => "Nie mo\x{17c}na od\x{142}\x{105}czy\x{107} link %s: %s"
  },
  'create package' => {
    'lastUpdated' => '1204059417',
    'message' => "Uczyni\x{107} dost\x{119}pne jako pakiet?"
  },
  'create shortcut' => {
    'lastUpdated' => '1236941405',
    'message' => 'Utwórz skrót'
  },
  'current url' => {
    'lastUpdated' => '1204059463',
    'message' => 'Aktualny URL'
  },
  'cut' => {
    'lastUpdated' => '1204059477',
    'message' => 'Wytnij'
  },
  'default value' => {
    'lastUpdated' => '1236941439',
    'message' => "Domy\x{15b}lna warto\x{15b}\x{107}(\x{15b}ci)"
  },
  'default value description' => {
    'lastUpdated' => '1236941459',
    'message' => "<p>Domy\x{15b}lna warto\x{15b}\x{107} dla tego pola. W przypadku wielu domy\x{15b}lnych warto\x{15b}ci, jak ma to miejsce w Check Box List, nale\x{17c}y wprowadzi\x{107} jedn\x{105} warto\x{15b}\x{107} w linii. Maksymalna liczba znak\x{f3}w wynosi 255 dla wszystkich linii tekstu.</p>"
  },
  'delete' => {
    'lastUpdated' => '1204059652',
    'message' => "Usu\x{144}"
  },
  'deleteConfirm' => {
    'lastUpdated' => '1236941467',
    'message' => "Czy jeste\x{15b} pewny, \x{17c}e chcesz usun\x{105}\x{107} w\x{142}asno\x{15b}\x{107} Metadata ?"
  },
  'demote' => {
    'lastUpdated' => '1204234568',
    'message' => 'Zdegraduj'
  },
  'demote help' => {
    'lastUpdated' => '1236941478',
    'message' => "Przesu\x{144} ten zas\x{f3}b do do\x{142}u o jedno miejsce na stronie."
  },
  'directory index' => {
    'lastUpdated' => '1204234596',
    'message' => 'Indeks Katalogu'
  },
  'directory index description' => {
    'lastUpdated' => '1236941496',
    'message' => "<p>Je\x{15b}li adres URL eksportowanego Zasobu/Asset wygl\x{105}da jak katalog, dodany b\x{119}dzie indeks katalogu do niego.</p>"
  },
  'does asset inherit URL from parent' => {
    'lastUpdated' => '1236941501',
    'message' => "Doda\x{107} z przodu URL rodzica?"
  },
  'does asset inherit URL from parent description' => {
    'lastUpdated' => '1236941521',
    'message' => "Czy zas\x{f3}b/asset ma poprzedza\x{107} sw\x{f3}j adres URL adresem URL rodzica?"
  },
  'done' => {
    'lastUpdated' => '1204234746',
    'message' => 'ZROBIONE'
  },
  'duplicate' => {
    'lastUpdated' => '1204234753',
    'message' => 'Duplikuj'
  },
  'duplicateField' => {
    'lastUpdated' => '1236941533',
    'message' => "<p><strong>B\x{142}\x{105}d: Pole &quot;%field%&quot; jest ju\x{17c} w u\x{17c}yciu.</strong></p>"
  },
  'edit' => {
    'lastUpdated' => '1204234798',
    'message' => 'Edytuj'
  },
  'edit branch' => {
    'lastUpdated' => '1204234834',
    'message' => "Edycja Ga\x{142}\x{119}zi"
  },
  'edit branch help' => {
    'lastUpdated' => '1236941551',
    'message' => "Uruchom interfejs Edycji Ga\x{142}\x{119}zi by wykona\x{107} zmiany na tym Zasobie/Asset i zasobach poni\x{17c}ej"
  },
  'edit branch url help' => {
    'lastUpdated' => '1236941613',
    'message' => "<p>Adres URL mo\x{17c}esz zmieni\x{107} wykorzystuj\x{105}c dwie sekcje: adres serwera (lewa sekcja) i \x{15b}cie\x{17c}ka pliku (prawa sekcja). Dla sekcji adresu serwera dost\x{119}pne s\x{105} trzy opcje:</p><p>Adres URL rodzica</p><p>&nbsp;&nbsp;&nbsp; Startuje ten adres URL z adresem rodzica</p><p>Okre\x{15b}lony</p><p>&nbsp;&nbsp;&nbsp; Wprowad\x{17a} now\x{105} baz\x{119} adresu w polu, kt\x{f3}re pojawi si\x{119} kiedy Specified Base zostanie wybrane.</p><p>\x{17b}aden</p><p>&nbsp;&nbsp;&nbsp; Wszystkie Zasoby/Asset maj\x{105} mie\x{107} adres URL &quot;top-level&quot; .</p><p>&nbsp;</p><p>Dost\x{119}pne s\x{105} r\x{f3}wnie\x{17c} trzy opcje dla adresu pliku:&nbsp; </p><p>Tytu\x{142} w menu</p><p>&nbsp;&nbsp;&nbsp; Nowy adres URL b\x{119}dzie bazowa\x{142} na polu tytu\x{142}u dla menu zasobu&nbsp;</p><p> Title</p><p>&nbsp;&nbsp;&nbsp; Nowy adres URL b\x{119}dzie bazowa\x{142} na polu tytu\x{142}u.</p><p>Aktualny adres URL</p><p>&nbsp;&nbsp;&nbsp;  Nowy adres URL b\x{119}dzie bazowa\x{142} na aktualnym adresie zasobu</p><p>&nbsp;</p><p>Nowe adresy URL b\x{119}d\x{105} przekazane przez funkcj\x{119} makeUrlCompliant, kt\x{f3}ra jest specyficzna dla ka\x{17c}dego j\x{119}zyka. </p><p>&nbsp;</p>"
  },
  'encrypt page' => {
    'lastUpdated' => '1236941889',
    'message' => "Szyfrowa\x{107} zawarto\x{15b}\x{107}?"
  },
  'encrypt page description' => {
    'lastUpdated' => '1236942080',
    'message' => "<p>Czy strona zawieraj\x{105}ca ten zas\x{f3}b/asset ma by\x{107} szyfrowana w SSL?</p>"
  },
  'encryptPage' => {
    'lastUpdated' => '1236942096',
    'message' => "<p>Je\x{15b}li Zas\x{f3}b/Asset ma by\x{107} szyfrowany przez SSL nale\x{17c}y ustawi\x{107} warto\x{15b}\x{107} TRUE.</p>"
  },
  'errorEmptyField' => {
    'lastUpdated' => '1236942101',
    'message' => "<p><strong>B\x{142}\x{105}d: Nazwa pola nie mo\x{17c}e by\x{107} pusta.</strong></p>"
  },
  'export' => {
    'lastUpdated' => '1227595503',
    'message' => "Wi\x{119}cej"
  },
  'export information' => {
    'lastUpdated' => '1236942110',
    'message' => '<p>Wyeksportowano %d stron w %d sekund.</p>'
  },
  'exporting page' => {
    'lastUpdated' => '1236942115',
    'message' => 'Eksport strony %-s ......'
  },
  'extra head tags' => {
    'lastUpdated' => '1236942132',
    'message' => 'Ekstra elementy &lt;head&gt;&nbsp; (tags)'
  },
  'extra head tags description' => {
    'lastUpdated' => '1236942145',
    'message' => "<p>Te znaczniki b\x{119}d\x{105} dodane do sekcji &lt;head&gt; w ka\x{17c}dej stronie, na kt\x{f3}rej pojawi si\x{119} Zas\x{f3}b/Asset.</p>"
  },
  'extraHeadTags' => {
    'lastUpdated' => '1236942171',
    'message' => "<p>Dodatkowe znaczniki b\x{119}d\x{105} dodane do nag\x{142}\x{f3}wka stron zawieraj\x{105}cych Zas\x{f3}b/Asset. Znaczniki b\x{119}d\x{105} dodane domy\x{15b}lnie, wi\x{119}c nie musisz dodawa\x{107} ich samemu. Chyba, \x{17c}e chcesz aby wyst\x{105}pi\x{142}y dwa razy.</p>"
  },
  'extrasUploads form hoverHelp' => {
    'lastUpdated' => '1236942265',
    'message' => "<p>Jak\x{105} nale\x{17c}y podj\x{105}\x{107} akcj\x{119} odno\x{15b}nie plik\x{f3}w w katalogach extras i uploads, kt\x{f3}re cz\x{119}sto stanowi\x{105} cz\x{119}\x{15b}\x{107} stron serwisu WWW. <b>Symlink</b> oznacza u\x{17c}ycie symbolicznych link\x{f3}w (nie dost\x{119}pnych we wszystkich systemach operacyjnych) do oryginalnego katalogu. <b>None</b> oznacza zignorowanie katalog\x{f3}w extras i uploads, co prawdopodobnie spowoduje z\x{142}amanie odno\x{15b}nik\x{f3}w w eksportowanym serwisie. Oczywi\x{15b}cie wy\x{142}\x{105}cznie, je\x{15b}li nie przygotujesz odpowiednio sam tych katalog\x{f3}w.</p>"
  },
  'extrasUploads form label' => {
    'lastUpdated' => '1236942296',
    'message' => "Katalogi dla dodatk\x{f3}w i przes\x{142}anych plik\x{f3}w"
  },
  'extrasUploads form option none' => {
    'lastUpdated' => '1236942304',
    'message' => 'None (bez linków)'
  },
  'extrasUploads form option symlink' => {
    'lastUpdated' => '1236942312',
    'message' => "Symlink (u\x{17c}ycie link\x{f3}w)"
  },
  'extrasUploads symlinking' => {
    'lastUpdated' => '1236942321',
    'message' => 'Linkowanie katalogów extras i uploads.'
  },
  'groupIdEdit' => {
    'lastUpdated' => '1236942359',
    'message' => "ID grupy, kt\x{f3}rej cz\x{142}onkowie posiadaj\x{105} uprawnienie do edycji zasobu (Asset)."
  },
  'groupIdView' => {
    'lastUpdated' => '1236942347',
    'message' => "ID grupy, kt\x{f3}rej cz\x{142}onkowie posiadaj\x{105} prawo do wy\x{15b}wietlenia zawarto\x{15b}ci zasobu (Asset)."
  },
  'import' => {
    'lastUpdated' => '1236942364',
    'message' => 'Import'
  },
  'inherit parent permissions' => {
    'lastUpdated' => '1236942372',
    'message' => 'Odziedzicz uprawnienia rodzica'
  },
  'is not a valid userId' => {
    'lastUpdated' => '1236942378',
    'message' => 'nie jest poprawnym userId'
  },
  'isHidden' => {
    'lastUpdated' => '1236942388',
    'message' => "Prze\x{142}\x{105}cznik wskazuj\x{105}cy, czy zas\x{f3}b (Asset) ma by\x{107} ukryty w elementach nawigacyjnych."
  },
  'isPackage' => {
    'lastUpdated' => '1236942396',
    'message' => "Prze\x{142}\x{105}cznik wskazuj\x{105}cy, czy zas\x{f3}b (Asset) jest pakietem."
  },
  'isPrototype' => {
    'lastUpdated' => '1236943001',
    'message' => "Prze\x{142}\x{105}cznik wskazuj\x{105}cym czy zas\x{f3}b (Asset) jest prototypem."
  },
  'keywords' => {
    'lastUpdated' => '1236943010',
    'message' => "S\x{142}owa kluczowe"
  },
  'keywords help' => {
    'lastUpdated' => '1236943047',
    'message' => "Wprowad\x{17a} kilka s\x{142}\x{f3}w kluczowych dla zasobu. Klucze b\x{119}d\x{105} automatycznie dodane do indeksu szukania, a tak\x{17c}e do sekcji meta w dokumentach HTML."
  },
  'last updated' => {
    'lastUpdated' => '1236943063',
    'message' => 'Ostatnio aktualizowany'
  },
  'lock' => {
    'lastUpdated' => '1227596792',
    'message' => 'Blokuj'
  },
  'lock help' => {
    'lastUpdated' => '1236943077',
    'message' => "Przejmij kopi\x{119} tego zasobu dla edycji. Nast\x{105}pi blokada zasobu dla wszystkich nie u\x{17c}ywaj\x{105}cych Twojej aktualnej wersji.  Opcja ta jest wy\x{15b}wietlana wy\x{142}\x{105}cznie je\x{15b}li zas\x{f3}b nie jest aktualnie zablokowany."
  },
  'locked' => {
    'lastUpdated' => '1227596799',
    'message' => 'Zablokowany'
  },
  'locked by' => {
    'lastUpdated' => '1236943124',
    'message' => 'Zablokowany przez'
  },
  'make asset exportable' => {
    'lastUpdated' => '1236943136',
    'message' => "Czy uczyni\x{107} zas\x{f3}b eksportowalnym?"
  },
  'make asset exportable description' => {
    'lastUpdated' => '1236943436',
    'message' => "Czy ten zas\x{f3}b mo\x{17c}e by\x{107} eksportowany w postaci statycznego dokumentu HTML, kt\x{f3}ry jest zupe\x{142}nie inny ni\x{17c} pakiet? Zas\x{f3}b i wszystkie powi\x{105}zane zasoby musz\x{105} by\x{107} eksportowalne. Dodatkowo musi by\x{107} w\x{142}\x{105}czony eksport w pliku konfiguracyjnym WebGUI."
  },
  'make package' => {
    'lastUpdated' => '1204238904',
    'message' => "Utworzy\x{107} pakiet?"
  },
  'make package description' => {
    'lastUpdated' => '1236943482',
    'message' => "<p>Wiele zada\x{144} powtarza si\x{119} w WebGUI. Automatyzacja takich zada\x{144} jak utworzenie Zasobu/Asset lub wykonanie r\x{f3}\x{17c}nych ustawie\x{144} wykonywane s\x{105} przez utworzone pakiety, kt\x{f3}re nast\x{119}pnie mog\x{105} by\x{107} wielokrotnie u\x{17c}yte w serwisie. Wybierz Tak, je\x{15b}li chcesz aby ten Zas\x{f3}b by\x{142} dost\x{119}pny w pakiecie. </p>"
  },
  'make prototype' => {
    'lastUpdated' => '1236943487',
    'message' => "Utworzy\x{107} prototyp?"
  },
  'make prototype description' => {
    'lastUpdated' => '1236943504',
    'message' => "<p>Ustaw ten Zas\x{f3}b/Asset jako Prototyp Zawarto\x{15b}ci aby inni mogli u\x{17c}y\x{107} go w twoim serwisie.</p>"
  },
  'manage' => {
    'lastUpdated' => '1204239696',
    'message' => "Zarz\x{105}dzaj"
  },
  'manage help' => {
    'lastUpdated' => 0,
    'message' => 'Bring up the Asset Manager displaying this Asset\'s children, if any.'
  },
  'menu label' => {
    'lastUpdated' => undef,
    'message' => ''
  },
  'menuTitle' => {
    'lastUpdated' => '1236943982',
    'message' => "Tytu\x{142} Zasobu u\x{17c}ywany w Nawigacjach"
  },
  'metadata edit property' => {
    'lastUpdated' => '1236943999',
    'message' => 'Metadata, Edycja'
  },
  'missing page query' => {
    'lastUpdated' => '1236944009',
    'message' => "Strona, kt\x{f3}rej za\x{17c}\x{105}dano nie istnieje. Co chcia\x{142}by\x{15b} zrobi\x{107}?"
  },
  'need a userId parameter' => {
    'lastUpdated' => '1236944238',
    'message' => 'Potrzebny parametr userId'
  },
  'newWindow' => {
    'lastUpdated' => '1236944530',
    'message' => "Warto\x{15b}\x{107} ta b\x{119}dzie wynosi\x{107} <b>TRUE</b> je\x{17c}eli Zas\x{f3}b/Asset ten jest otwierany w nowym oknie przegl\x{105}darki."
  },
  'no results' => {
    'lastUpdated' => '1236944536',
    'message' => 'Nic nie znaleziono.'
  },
  'none' => {
    'lastUpdated' => '1236944557',
    'message' => 'None'
  },
  'over max assets' => {
    'lastUpdated' => '1236944580',
    'message' => "Tw\x{f3}j administrator okre\x{15b}li\x{142} limit ilo\x{15b}ci zasob\x{f3}w jakie mo\x{17c}esz zamie\x{15b}ci\x{107} w serwisie WWW. Niestety limit zosta\x{142} przekroczony. Usu\x{144} kilka starych zasob\x{f3}w w celu dodania nowych."
  },
  'ownerUserId' => {
    'lastUpdated' => '1236944586',
    'message' => "ID u\x{17c}ytkownika, kt\x{f3}ry jest w\x{142}a\x{15b}cicielem zasobu."
  },
  'package corrupt' => {
    'lastUpdated' => '1236944613',
    'message' => "Pakiet, kt\x{f3}ry pr\x{f3}bujesz zaimportowa\x{107} wydaje si\x{119} by\x{107} zepsuty. Import przebiegnie do miejsca, w kt\x{f3}rym wykryto b\x{142}\x{105}d. Je\x{15b}li nie chcesz zaimportowanej cz\x{119}\x{15b}ci pakietu wykonaj wycofanie twojej przywieszki wersji."
  },
  'package extract error' => {
    'lastUpdated' => '1236944662',
    'message' => "Nie uda\x{142}o si\x{119} rozpakowa\x{107} pakietu. Archiwum mo\x{17c}e by\x{107} uszkodzone, lub wyst\x{105}pi\x{142} b\x{142}\x{105}d serwera uniemo\x{17c}liwiaj\x{105}cy import pakiet\x{f3}w."
  },
  'packages' => {
    'lastUpdated' => '1204239857',
    'message' => 'Pakiety'
  },
  'page indicator' => {
    'lastUpdated' => '1227965973',
    'message' => 'Strona %s z %s'
  },
  'parent url' => {
    'lastUpdated' => '1227965663',
    'message' => "Nadrz\x{119}dny URL"
  },
  'promote' => {
    'lastUpdated' => '1214817333',
    'message' => "Przesu\x{144} wy\x{17c}ej"
  },
  'promote help' => {
    'lastUpdated' => '1236944682',
    'message' => "Przesu\x{144} ten zas\x{f3}b do g\x{f3}ry o jedno miejsce na stronie."
  },
  'properties' => {
    'lastUpdated' => '1214817305',
    'message' => "W\x{142}a\x{15b}ciwo\x{15b}ci"
  },
  'prototype using title' => {
    'lastUpdated' => '1236944717',
    'message' => "U\x{17c}ycie prototyp\x{f3}w zawarto\x{15b}ci"
  },
  'purge' => {
    'lastUpdated' => '1204239376',
    'message' => "Wyczy\x{15b}\x{107}"
  },
  'purge old asset revisions' => {
    'lastUpdated' => '1236944728',
    'message' => "Wyczy\x{15b}ci\x{107} stare rewizje zasob\x{f3}w"
  },
  'purge old trash' => {
    'lastUpdated' => '1236944733',
    'message' => "Wyczy\x{15b}\x{107} stare \x{15b}mieci"
  },
  'purge revision after' => {
    'lastUpdated' => '1236944748',
    'message' => "Wyczy\x{15b}\x{107} stare rewizje zasob\x{f3}w po"
  },
  'purge revision after help' => {
    'lastUpdated' => '1236944771',
    'message' => "Jak d\x{142}ugo powiny by\x{107} przechowywane stare rewizje zasobu/asset? Stare rewizje zasobu to te, kt\x{f3}re nie by\x{142}y od d\x{142}u\x{17c}szego czasu ogl\x{105}dane przez u\x{17c}ytkownik\x{f3}w, ale s\x{105} przechowywane w systemie wersji dla cofni\x{119}cia zmian."
  },
  'purge revision prompt' => {
    'lastUpdated' => '1236944784',
    'message' => "Czy na pewno \x{17c}yczysz sobie usun\x{105}\x{107} rewizj\x{119} dla tego zasobu? Po usuni\x{119}ciu nie b\x{119}dzie mo\x{17c}liwe przywr\x{f3}cenie."
  },
  'purge trash after' => {
    'lastUpdated' => '1204239405',
    'message' => "Wyczy\x{15b}\x{107} stare \x{15b}mieci po"
  },
  'purge trash after help' => {
    'lastUpdated' => '1236944806',
    'message' => "Jak d\x{142}ugo zas\x{f3}b powinien pozosta\x{107} w koszu, aby zosta\x{107} uznanym za dostatecznie stary, aby go usun\x{105}\x{107}? Zauwa\x{17c}, \x{17c}e kiedy zostanie usuni\x{119}ty, wszystkie rewizje i potomki r\x{f3}wnie\x{17c} zostan\x{105} usuni\x{119}te."
  },
  'rank' => {
    'lastUpdated' => '1204239441',
    'message' => 'Ranga'
  },
  'restore' => {
    'lastUpdated' => '1236944815',
    'message' => "Przywr\x{f3}\x{107}"
  },
  'revised by' => {
    'lastUpdated' => '1236944819',
    'message' => 'Zrewidowany przez'
  },
  'revision date' => {
    'lastUpdated' => '1204239462',
    'message' => 'Data rewizji'
  },
  'revisions' => {
    'lastUpdated' => '1236944824',
    'message' => 'Rewizje'
  },
  'revisions help' => {
    'lastUpdated' => '1236944829',
    'message' => "Poka\x{17c} list\x{119} wszystkich rewizji dla tego zasobu."
  },
  'rootUrl default not present' => {
    'lastUpdated' => '1236946013',
    'message' => 'Not symlinking default asset; not included in exported subtree.'
  },
  'rootUrl form hoverHelp' => {
    'lastUpdated' => '1160773957',
    'message' => 'What action to take regarding queries to the root URL.  Symlink Default means to create a symbolic link from the root-URL index file to the index file of the default asset (not available on all systems).  None means to do nothing, which usually causes queries to the root URL to be rejected in the exported site.'
  },
  'rootUrl form label' => {
    'lastUpdated' => '1204239536',
    'message' => 'Adres URL korzenia'
  },
  'rootUrl form option none' => {
    'lastUpdated' => '1160773957',
    'message' => 'None'
  },
  'rootUrl form option symlinkDefault' => {
    'lastUpdated' => '1160773957',
    'message' => 'Symlink Default'
  },
  'rootUrl symlinking default' => {
    'lastUpdated' => '1160773957',
    'message' => 'Symlinking extras and uploads dirs.'
  },
  'save and commit' => {
    'lastUpdated' => '1204239547',
    'message' => "zapisz &amp; zatwierd\x{17a}"
  },
  'search' => {
    'lastUpdated' => '1227965813',
    'message' => 'Szukaj'
  },
  'select all' => {
    'lastUpdated' => '1236946088',
    'message' => 'Wybierz wszystko'
  },
  'size' => {
    'lastUpdated' => '1204239497',
    'message' => 'Rozmiar'
  },
  'specified base' => {
    'lastUpdated' => '1236945919',
    'message' => 'Specified Base'
  },
  'status' => {
    'lastUpdated' => '1236946310',
    'message' => 'With respect to version control, the status of this Asset.  Typically these are the English strings "approved", "pending", "committed".'
  },
  'synopsis' => {
    'lastUpdated' => '1236946077',
    'message' => "Kr\x{f3}tki opis zawarto\x{15b}ci zasobu."
  },
  'tag name' => {
    'lastUpdated' => '1236946345',
    'message' => 'Nazwa przywieszki'
  },
  'this asset only' => {
    'lastUpdated' => '1236946381',
    'message' => 'Tylko Ten Zasób/Asset'
  },
  'title' => {
    'lastUpdated' => '1204236151',
    'message' => "Tytu\x{142} zasobu"
  },
  'topicName' => {
    'lastUpdated' => '1204236170',
    'message' => 'Zasoby/Assets'
  },
  'type' => {
    'lastUpdated' => '1204236176',
    'message' => 'Typ'
  },
  'unlocked' => {
    'lastUpdated' => '1236946329',
    'message' => 'Odblokowany'
  },
  'update' => {
    'lastUpdated' => '1227966097',
    'message' => 'Zapisz'
  },
  'url' => {
    'lastUpdated' => '1204236181',
    'message' => 'Adres URL zasobu.'
  },
  'view' => {
    'lastUpdated' => '1204236187',
    'message' => "Poka\x{17c}"
  },
  'view help' => {
    'lastUpdated' => '1236946420',
    'message' => "Wiele Zasob\x{f3}w/Assets pokazywanych jest jako cz\x{119}\x{15b}\x{107} pojemnika - zasobu typu Strona lub Folder. Ta opcja pozwoli wy\x{15b}wietli\x{107} Zas\x{f3}b samodzielnie."
  },
  'with children' => {
    'lastUpdated' => '1236946426',
    'message' => "Z Dzie\x{107}mi"
  },
  'with descendants' => {
    'lastUpdated' => '1236946434',
    'message' => 'Z Potomkami'
  },
  'with selected' => {
    'lastUpdated' => '1236947207',
    'message' => 'Wybrane: &nbsp;'
  }
}
;

1;
