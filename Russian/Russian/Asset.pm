package WebGUI::i18n::Russian::Asset;

our $I18N = {
          'lock help' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          'make package' => {
                              'lastUpdated' => undef,
                              'message' => ''
                            },
          '959' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'Uploads URL' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          'Export as user description' => {
                                            'lastUpdated' => undef,
                                            'message' => ''
                                          },
          'parent url' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          'keywords' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'make asset exportable' => {
                                       'lastUpdated' => undef,
                                       'message' => ''
                                     },
          'newWindow' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          '871 description' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'add the missing page' => {
                                      'lastUpdated' => undef,
                                      'message' => ''
                                    },
          'view' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          'encrypt page description' => {
                                          'lastUpdated' => undef,
                                          'message' => ''
                                        },
          'controls' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'export information' => {
                                    'lastUpdated' => undef,
                                    'message' => ''
                                  },
          'isHidden' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'save and commit' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'asset' => {
                       'lastUpdated' => undef,
                       'message' => ''
                     },
          'Data Type description' => {
                                       'lastUpdated' => undef,
                                       'message' => ''
                                     },
          '872' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'promote help' => {
                              'lastUpdated' => undef,
                              'message' => ''
                            },
          'properties' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          'delete' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'title' => {
                       'lastUpdated' => undef,
                       'message' => ''
                     },
          'revised by' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          '11' => {
                    'lastUpdated' => 1149246696,
                    'message' => 'Очистить мусорник'
                  },
          'could not open path' => {
                                     'lastUpdated' => undef,
                                     'message' => ''
                                   },
          'Edit Metadata' => {
                               'lastUpdated' => undef,
                               'message' => ''
                             },
          'extra head tags' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'default value' => {
                               'lastUpdated' => undef,
                               'message' => ''
                             },
          'Depth' => {
                       'lastUpdated' => undef,
                       'message' => ''
                     },
          '412 description' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          '964' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'locked' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'create package' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'Export Page' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          'with children' => {
                               'lastUpdated' => undef,
                               'message' => ''
                             },
          'purge trash after help' => {
                                        'lastUpdated' => undef,
                                        'message' => ''
                                      },
          'rootUrl symlinking default' => {
                                            'lastUpdated' => undef,
                                            'message' => ''
                                          },
          'assets' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'view help' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          '886' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'deleteConfirm' => {
                               'lastUpdated' => undef,
                               'message' => ''
                             },
          'make asset exportable description' => {
                                                   'lastUpdated' => undef,
                                                   'message' => ''
                                                 },
          'manage help' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          '1079' => {
                      'lastUpdated' => 1149246187,
                      'message' => 'Шаблон версии для печати'
                    },
          'edit branch url help' => {
                                      'lastUpdated' => undef,
                                      'message' => ''
                                    },
          'asset id description' => {
                                      'lastUpdated' => undef,
                                      'message' => ''
                                    },
          'Possible Values description' => {
                                             'lastUpdated' => undef,
                                             'message' => ''
                                           },
          'restore' => {
                         'lastUpdated' => undef,
                         'message' => ''
                       },
          'Extras URL description' => {
                                        'lastUpdated' => undef,
                                        'message' => ''
                                      },
          'copy' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          'synopsis' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'extraHeadTags' => {
                               'lastUpdated' => undef,
                               'message' => ''
                             },
          'duplicate' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          'isPrototype' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          '1083' => {
                      'lastUpdated' => 1149246633,
                      'message' => 'Новое Содержимое'
                    },
          'Field Id' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'create shortcut' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'Extras URL' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          '966' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'demote' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'extrasUploads form option symlink' => {
                                                   'lastUpdated' => undef,
                                                   'message' => ''
                                                 },
          'change url' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          'purge revision after help' => {
                                           'lastUpdated' => undef,
                                           'message' => ''
                                         },
          '895' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'committed versions' => {
                                    'lastUpdated' => undef,
                                    'message' => ''
                                  },
          'revisions help' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'Export as user' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'type' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          'asset list title' => {
                                  'lastUpdated' => undef,
                                  'message' => ''
                                },
          'with descendants' => {
                                  'lastUpdated' => undef,
                                  'message' => ''
                                },
          'Add new field description' => {
                                           'lastUpdated' => undef,
                                           'message' => ''
                                         },
          'revisions' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          '104' => {
                     'lastUpdated' => 1149246028,
                     'message' => 'URL'
                   },
          'edit branch help' => {
                                  'lastUpdated' => undef,
                                  'message' => ''
                                },
          '412' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          '954' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'status' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'assetSize' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          'purge revision after' => {
                                      'lastUpdated' => undef,
                                      'message' => ''
                                    },
          'asset template asset var title' => {
                                                'lastUpdated' => undef,
                                                'message' => ''
                                              },
          '1082' => {
                      'lastUpdated' => 1149246274,
                      'message' => 'Буфер обмена'
                    },
          'cut' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          '487' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'rootUrl form option symlinkDefault' => {
                                                    'lastUpdated' => undef,
                                                    'message' => ''
                                                  },
          'isPackage' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          'Metadata Description description' => {
                                                  'lastUpdated' => undef,
                                                  'message' => ''
                                                },
          'Delete Metadata field' => {
                                       'lastUpdated' => undef,
                                       'message' => ''
                                     },
          'default value description' => {
                                           'lastUpdated' => undef,
                                           'message' => ''
                                         },
          'could not rmdir' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          '950' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          '486' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'this asset only' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'confirm change' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'import' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'extra head tags description' => {
                                             'lastUpdated' => undef,
                                             'message' => ''
                                           },
          '411 description' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'purge old trash' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'Field Name description' => {
                                        'lastUpdated' => undef,
                                        'message' => ''
                                      },
          '43' => {
                    'lastUpdated' => 1213980312,
                    'message' => 'Вы действительно хотите удалить эту часть? У этого блока откат невозможен и содержимое будет удалено немедленно.'
                  },
          '940' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'rank' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          '886 description' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'directory index' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'extrasUploads form option none' => {
                                                'lastUpdated' => undef,
                                                'message' => ''
                                              },
          '99' => {
                    'lastUpdated' => undef,
                    'message' => ''
                  },
          'Page Export Status' => {
                                    'lastUpdated' => undef,
                                    'message' => ''
                                  },
          'could not unlink' => {
                                  'lastUpdated' => undef,
                                  'message' => ''
                                },
          'url' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'Class Icon' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          'groupIdEdit' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          'Class Icon help' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'tag name' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'change' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'current url' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          '108' => {
                     'lastUpdated' => 1149246200,
                     'message' => 'Владелец'
                   },
          'could not create path' => {
                                       'lastUpdated' => undef,
                                       'message' => ''
                                     },
          'could not create' => {
                                  'lastUpdated' => undef,
                                  'message' => ''
                                },
          'purge trash after' => {
                                   'lastUpdated' => undef,
                                   'message' => ''
                                 },
          'Add new field' => {
                               'lastUpdated' => undef,
                               'message' => ''
                             },
          '871' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'Page Export' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          'over max assets' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'manage' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'none' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          'make package description' => {
                                          'lastUpdated' => undef,
                                          'message' => ''
                                        },
          'duplicateField' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'edit branch' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          '108 description' => {
                                 'lastUpdated' => 1149246220,
                                 'message' => '<p>Владелец компонента (asset) обычно человек, который создал этот компонент (asset). Этот пользователь имеет все права для редактирования и установления прав доступа на этот компонент (asset). <em>UI уровень: 6</em></p><p><strong>ПРИМЕЧАНИЕ</strong>: Владелец может быть изменён администратором.</p>'
                               },
          'make prototype description' => {
                                            'lastUpdated' => undef,
                                            'message' => ''
                                          },
          'rootUrl form option none' => {
                                          'lastUpdated' => undef,
                                          'message' => ''
                                        },
          'select all' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          'edit' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          '107' => {
                     'lastUpdated' => 1149246165,
                     'message' => 'Права доступа'
                   },
          'size' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          'confirm change url message' => {
                                            'lastUpdated' => undef,
                                            'message' => ''
                                          },
          '174' => {
                     'lastUpdated' => 1210106498,
                     'message' => 'Отобразить заголовок?'
                   },
          'rootUrl form label' => {
                                    'lastUpdated' => undef,
                                    'message' => ''
                                  },
          '940 description' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'last updated' => {
                              'lastUpdated' => undef,
                              'message' => ''
                            },
          'Select' => {
                        'lastUpdated' => undef,
                        'message' => ''
                      },
          'package extract error' => {
                                       'lastUpdated' => undef,
                                       'message' => ''
                                     },
          'prototype using title' => {
                                       'lastUpdated' => undef,
                                       'message' => ''
                                     },
          'encryptPage' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          'ago' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'revision date' => {
                               'lastUpdated' => undef,
                               'message' => ''
                             },
          'encrypt page' => {
                              'lastUpdated' => undef,
                              'message' => ''
                            },
          'Uploads URL description' => {
                                         'lastUpdated' => undef,
                                         'message' => ''
                                       },
          'specified base' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'asset template title' => {
                                      'lastUpdated' => undef,
                                      'message' => ''
                                    },
          '949' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'demote help' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          '104 description' => {
                                 'lastUpdated' => 1149246052,
                                 'message' => 'URL для этого компонента (asset) должен быть уникальным. Если оставить это поле незаполненным, то URL будет создан автоматически путём добавления к родительскому URL названия из меню. Если название содержит символы кириллицы, то они будут транслитерированы в латинские.'
                               },
          'extrasUploads form label' => {
                                          'lastUpdated' => undef,
                                          'message' => ''
                                        },
          'done' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          'lock' => {
                      'lastUpdated' => undef,
                      'message' => ''
                    },
          'rootUrl default not present' => {
                                             'lastUpdated' => undef,
                                             'message' => ''
                                           },
          'Metadata' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'Field name' => {
                            'lastUpdated' => undef,
                            'message' => ''
                          },
          'missing page query' => {
                                    'lastUpdated' => undef,
                                    'message' => ''
                                  },
          'package corrupt' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'could not symlink' => {
                                   'lastUpdated' => undef,
                                   'message' => ''
                                 },
          'ownerUserId' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          'content profiling' => {
                                   'lastUpdated' => undef,
                                   'message' => ''
                                 },
          '965' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'errorEmptyField' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'purge old asset revisions' => {
                                           'lastUpdated' => undef,
                                           'message' => ''
                                         },
          'bad user privileges' => {
                                     'lastUpdated' => undef,
                                     'message' => ''
                                   },
          'directory index description' => {
                                             'lastUpdated' => undef,
                                             'message' => ''
                                           },
          'promote' => {
                         'lastUpdated' => undef,
                         'message' => ''
                       },
          '896' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          'groupIdView' => {
                             'lastUpdated' => undef,
                             'message' => ''
                           },
          'Depth description' => {
                                   'lastUpdated' => undef,
                                   'message' => ''
                                 },
          'extrasUploads symlinking' => {
                                          'lastUpdated' => undef,
                                          'message' => ''
                                        },
          'extrasUploads form hoverHelp' => {
                                              'lastUpdated' => undef,
                                              'message' => ''
                                            },
          'make prototype' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'menuTitle' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          'topicName' => {
                           'lastUpdated' => undef,
                           'message' => ''
                         },
          'asset id' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'metadata edit property' => {
                                        'lastUpdated' => undef,
                                        'message' => ''
                                      },
          'purge' => {
                       'lastUpdated' => undef,
                       'message' => ''
                     },
          '1073' => {
                      'lastUpdated' => 1149246176,
                      'message' => 'Шаблон стиля (для страницы)'
                    },
          '105' => {
                     'lastUpdated' => 1149246149,
                     'message' => 'Визуализация'
                   },
          'rootUrl form hoverHelp' => {
                                        'lastUpdated' => undef,
                                        'message' => ''
                                      },
          '951' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   },
          '85' => {
                    'lastUpdated' => undef,
                    'message' => ''
                  },
          'exporting page' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          '872 description' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          '99 description' => {
                                'lastUpdated' => undef,
                                'message' => ''
                              },
          'change url help' => {
                                 'lastUpdated' => undef,
                                 'message' => ''
                               },
          'packages' => {
                          'lastUpdated' => undef,
                          'message' => ''
                        },
          'purge revision prompt' => {
                                       'lastUpdated' => undef,
                                       'message' => ''
                                     },
          '411' => {
                     'lastUpdated' => undef,
                     'message' => ''
                   }
        }
;

1;
