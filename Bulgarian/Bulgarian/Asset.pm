package WebGUI::i18n::Bulgarian::Asset;

our $I18N = {
	'lock help' => {
		message => q||,
		lastUpdated => 
	},

	'make package' => {
		message => q||,
		lastUpdated => 
	},

	'959' => {
		message => q||,
		lastUpdated => 
	},

	'parent url' => {
		message => q||,
		lastUpdated => 
	},

	'Export as user description' => {
		message => q||,
		lastUpdated => 
	},

	'Uploads URL' => {
		message => q||,
		lastUpdated => 
	},

	'871 description' => {
		message => q||,
		lastUpdated => 
	},

	'add the missing page' => {
		message => q||,
		lastUpdated => 
	},

	'view' => {
		message => q||,
		lastUpdated => 
	},

	'export information' => {
		message => q||,
		lastUpdated => 
	},

	'controls' => {
		message => q||,
		lastUpdated => 
	},

	'encrypt page description' => {
		message => q||,
		lastUpdated => 
	},

	'asset' => {
		message => q||,
		lastUpdated => 
	},

	'save and commit' => {
		message => q||,
		lastUpdated => 
	},

	'asset list body' => {
		message => q||,
		lastUpdated => 
	},

	'872' => {
		message => q||,
		lastUpdated => 
	},

	'Data Type description' => {
		message => q||,
		lastUpdated => 
	},

	'promote help' => {
		message => q||,
		lastUpdated => 
	},

	'properties' => {
		message => q||,
		lastUpdated => 
	},

	'delete' => {
		message => q||,
		lastUpdated => 
	},

	'revised by' => {
		message => q||,
		lastUpdated => 
	},

	'11' => {
		message => q||,
		lastUpdated => 
	},

	'could not open path' => {
		message => q||,
		lastUpdated => 
	},

	'Edit Metadata' => {
		message => q||,
		lastUpdated => 
	},

	'extra head tags' => {
		message => q||,
		lastUpdated => 
	},

	'Depth' => {
		message => q||,
		lastUpdated => 
	},

	'964' => {
		message => q||,
		lastUpdated => 
	},

	'412 description' => {
		message => q||,
		lastUpdated => 
	},

	'Export Page' => {
		message => q||,
		lastUpdated => 
	},

	'create package' => {
		message => q||,
		lastUpdated => 
	},

	'locked' => {
		message => q||,
		lastUpdated => 
	},

	'purge trash after help' => {
		message => q||,
		lastUpdated => 
	},

	'rootUrl symlinking default' => {
		message => q||,
		lastUpdated => 
	},

	'assets' => {
		message => q||,
		lastUpdated => 
	},

	'886' => {
		message => q||,
		lastUpdated => 
	},

	'view help' => {
		message => q||,
		lastUpdated => 
	},

	'deleteConfirm' => {
		message => q||,
		lastUpdated => 
	},

	'manage help' => {
		message => q||,
		lastUpdated => 
	},

	'1079' => {
		message => q||,
		lastUpdated => 
	},

	'edit branch body' => {
		message => q||,
		lastUpdated => 
	},

	'edit branch url help' => {
		message => q||,
		lastUpdated => 
	},

	'asset id description' => {
		message => q||,
		lastUpdated => 
	},

	'Possible Values description' => {
		message => q||,
		lastUpdated => 
	},

	'restore' => {
		message => q||,
		lastUpdated => 
	},

	'Extras URL description' => {
		message => q||,
		lastUpdated => 
	},

	'copy' => {
		message => q||,
		lastUpdated => 
	},

	'asset template body' => {
		message => q||,
		lastUpdated => 
	},

	'duplicate' => {
		message => q||,
		lastUpdated => 
	},

	'1083' => {
		message => q||,
		lastUpdated => 
	},

	'Field Id' => {
		message => q||,
		lastUpdated => 
	},

	'create shortcut' => {
		message => q||,
		lastUpdated => 
	},

	'Extras URL' => {
		message => q||,
		lastUpdated => 
	},

	'966' => {
		message => q||,
		lastUpdated => 
	},

	'demote' => {
		message => q||,
		lastUpdated => 
	},

	'extrasUploads form option symlink' => {
		message => q||,
		lastUpdated => 
	},

	'change url' => {
		message => q||,
		lastUpdated => 
	},

	'purge revision after help' => {
		message => q||,
		lastUpdated => 
	},

	'committed versions' => {
		message => q||,
		lastUpdated => 
	},

	'895' => {
		message => q||,
		lastUpdated => 
	},

	'Export as user' => {
		message => q||,
		lastUpdated => 
	},

	'revisions help' => {
		message => q||,
		lastUpdated => 
	},

	'asset list title' => {
		message => q||,
		lastUpdated => 
	},

	'type' => {
		message => q||,
		lastUpdated => 
	},

	'Add new field description' => {
		message => q||,
		lastUpdated => 
	},

	'104' => {
		message => q|URL|,
		lastUpdated => 1163703553
	},

	'revisions' => {
		message => q||,
		lastUpdated => 
	},

	'412' => {
		message => q||,
		lastUpdated => 
	},

	'edit branch help' => {
		message => q||,
		lastUpdated => 
	},

	'manage versions body' => {
		message => q||,
		lastUpdated => 
	},

	'954' => {
		message => q||,
		lastUpdated => 
	},

	'purge revision after' => {
		message => q||,
		lastUpdated => 
	},

	'1082' => {
		message => q||,
		lastUpdated => 
	},

	'asset fields body' => {
		message => q||,
		lastUpdated => 
	},

	'cut' => {
		message => q||,
		lastUpdated => 
	},

	'487' => {
		message => q||,
		lastUpdated => 
	},

	'rootUrl form option symlinkDefault' => {
		message => q||,
		lastUpdated => 
	},

	'Metadata Description description' => {
		message => q||,
		lastUpdated => 
	},

	'asset fields title' => {
		message => q||,
		lastUpdated => 
	},

	'could not rmdir' => {
		message => q||,
		lastUpdated => 
	},

	'Delete Metadata field' => {
		message => q||,
		lastUpdated => 
	},

	'950' => {
		message => q||,
		lastUpdated => 
	},

	'486' => {
		message => q||,
		lastUpdated => 
	},

	'import' => {
		message => q||,
		lastUpdated => 
	},

	'confirm change' => {
		message => q||,
		lastUpdated => 
	},

	'extra head tags description' => {
		message => q||,
		lastUpdated => 
	},

	'411 description' => {
		message => q||,
		lastUpdated => 
	},

	'purge old trash' => {
		message => q||,
		lastUpdated => 
	},

	'Field Name description' => {
		message => q||,
		lastUpdated => 
	},

	'940' => {
		message => q||,
		lastUpdated => 
	},

	'43' => {
		message => q||,
		lastUpdated => 
	},

	'rank' => {
		message => q||,
		lastUpdated => 
	},

	'886 description' => {
		message => q||,
		lastUpdated => 
	},

	'directory index' => {
		message => q||,
		lastUpdated => 
	},

	'extrasUploads form option none' => {
		message => q||,
		lastUpdated => 
	},

	'99' => {
		message => q||,
		lastUpdated => 
	},

	'Page Export Status' => {
		message => q||,
		lastUpdated => 
	},

	'could not unlink' => {
		message => q||,
		lastUpdated => 
	},

	'Page Export body' => {
		message => q||,
		lastUpdated => 
	},

	'Class Icon' => {
		message => q||,
		lastUpdated => 
	},

	'Class Icon help' => {
		message => q||,
		lastUpdated => 
	},

	'tag name' => {
		message => q||,
		lastUpdated => 
	},

	'change' => {
		message => q||,
		lastUpdated => 
	},

	'current url' => {
		message => q||,
		lastUpdated => 
	},

	'108' => {
		message => q||,
		lastUpdated => 
	},

	'could not create path' => {
		message => q||,
		lastUpdated => 
	},

	'could not create' => {
		message => q||,
		lastUpdated => 
	},

	'purge trash after' => {
		message => q||,
		lastUpdated => 
	},

	'Add new field' => {
		message => q||,
		lastUpdated => 
	},

	'871' => {
		message => q||,
		lastUpdated => 
	},

	'Page Export' => {
		message => q||,
		lastUpdated => 
	},

	'over max assets' => {
		message => q||,
		lastUpdated => 
	},

	'manage' => {
		message => q||,
		lastUpdated => 
	},

	'none' => {
		message => q||,
		lastUpdated => 
	},

	'make package description' => {
		message => q||,
		lastUpdated => 
	},

	'duplicateField' => {
		message => q||,
		lastUpdated => 
	},

	'edit branch' => {
		message => q||,
		lastUpdated => 
	},

	'108 description' => {
		message => q||,
		lastUpdated => 
	},

	'make prototype description' => {
		message => q||,
		lastUpdated => 
	},

	'rootUrl form option none' => {
		message => q||,
		lastUpdated => 
	},

	'edit' => {
		message => q||,
		lastUpdated => 
	},

	'select all' => {
		message => q||,
		lastUpdated => 
	},

	'asset toolbar body' => {
		message => q||,
		lastUpdated => 
	},

	'size' => {
		message => q||,
		lastUpdated => 
	},

	'change url body' => {
		message => q||,
		lastUpdated => 
	},

	'107' => {
		message => q||,
		lastUpdated => 
	},

	'confirm change url message' => {
		message => q||,
		lastUpdated => 
	},

	'rootUrl form label' => {
		message => q||,
		lastUpdated => 
	},

	'174' => {
		message => q||,
		lastUpdated => 
	},

	'940 description' => {
		message => q||,
		lastUpdated => 
	},

	'last updated' => {
		message => q||,
		lastUpdated => 
	},

	'Select' => {
		message => q||,
		lastUpdated => 
	},

	'prototype using title' => {
		message => q||,
		lastUpdated => 
	},

	'ago' => {
		message => q||,
		lastUpdated => 
	},

	'encrypt page' => {
		message => q||,
		lastUpdated => 
	},

	'revision date' => {
		message => q||,
		lastUpdated => 
	},

	'specified base' => {
		message => q||,
		lastUpdated => 
	},

	'Uploads URL description' => {
		message => q||,
		lastUpdated => 
	},

	'949' => {
		message => q||,
		lastUpdated => 
	},

	'asset template title' => {
		message => q||,
		lastUpdated => 
	},

	'demote help' => {
		message => q||,
		lastUpdated => 
	},

	'104 description' => {
		message => q|URL-то за този компонент (asset) трябва да е уникален. Ако това поле се остави празно, то URL-то ще бъде създадено от родителското URL и <strong>Заглавието на Менюто</strong>. Ако заглавието на менюто има кирилски символи, то те ще бъдат преобразувани в латински.|,
		lastUpdated => 1163703887
	},

	'extrasUploads form label' => {
		message => q||,
		lastUpdated => 
	},

	'done' => {
		message => q||,
		lastUpdated => 
	},

	'lock' => {
		message => q||,
		lastUpdated => 
	},

	'rootUrl default not present' => {
		message => q||,
		lastUpdated => 
	},

	'Metadata' => {
		message => q||,
		lastUpdated => 
	},

	'Field name' => {
		message => q||,
		lastUpdated => 
	},

	'missing page query' => {
		message => q||,
		lastUpdated => 
	},

	'package corrupt' => {
		message => q||,
		lastUpdated => 
	},

	'could not symlink' => {
		message => q||,
		lastUpdated => 
	},

	'content profiling' => {
		message => q||,
		lastUpdated => 
	},

	'965' => {
		message => q||,
		lastUpdated => 
	},

	'errorEmptyField' => {
		message => q||,
		lastUpdated => 
	},

	'purge old asset revisions' => {
		message => q||,
		lastUpdated => 
	},

	'bad user privileges' => {
		message => q||,
		lastUpdated => 
	},

	'directory index description' => {
		message => q||,
		lastUpdated => 
	},

	'promote' => {
		message => q||,
		lastUpdated => 
	},

	'896' => {
		message => q||,
		lastUpdated => 
	},

	'metadata manage body' => {
		message => q||,
		lastUpdated => 
	},

	'Depth description' => {
		message => q||,
		lastUpdated => 
	},

	'extrasUploads symlinking' => {
		message => q||,
		lastUpdated => 
	},

	'extrasUploads form hoverHelp' => {
		message => q||,
		lastUpdated => 
	},

	'make prototype' => {
		message => q||,
		lastUpdated => 
	},

	'topicName' => {
		message => q||,
		lastUpdated => 
	},

	'prototype using body' => {
		message => q||,
		lastUpdated => 
	},

	'asset id' => {
		message => q||,
		lastUpdated => 
	},

	'metadata edit property' => {
		message => q||,
		lastUpdated => 
	},

	'purge' => {
		message => q||,
		lastUpdated => 
	},

	'rootUrl form hoverHelp' => {
		message => q||,
		lastUpdated => 
	},

	'105' => {
		message => q|Визуализация|,
		lastUpdated => 1163703969
	},

	'1073' => {
		message => q||,
		lastUpdated => 
	},

	'asset toolbar title' => {
		message => q||,
		lastUpdated => 
	},

	'951' => {
		message => q||,
		lastUpdated => 
	},

	'85' => {
		message => q||,
		lastUpdated => 
	},

	'exporting page' => {
		message => q||,
		lastUpdated => 
	},

	'872 description' => {
		message => q||,
		lastUpdated => 
	},

	'99 description' => {
		message => q||,
		lastUpdated => 
	},

	'packages' => {
		message => q||,
		lastUpdated => 
	},

	'change url help' => {
		message => q||,
		lastUpdated => 
	},

	'metadata edit property body' => {
		message => q||,
		lastUpdated => 
	},

	'purge revision prompt' => {
		message => q||,
		lastUpdated => 
	},

	'411' => {
		message => q||,
		lastUpdated => 
	},

};

1;
