package WebGUI::i18n::Slovenian::WebGUI;

our $I18N = {
	'559' => {
		message => q||,
		lastUpdated => 
	},

	'84 description' => {
		message => q||,
		lastUpdated => 
	},

	'127' => {
		message => q|URL podjetja|,
		lastUpdated => 1178734161
	},

	'1049' => {
		message => q||,
		lastUpdated => 
	},

	'71' => {
		message => q||,
		lastUpdated => 
	},

	'118' => {
		message => q||,
		lastUpdated => 
	},

	'443' => {
		message => q||,
		lastUpdated => 
	},

	'959' => {
		message => q||,
		lastUpdated => 
	},

	'show all fields' => {
		message => q||,
		lastUpdated => 
	},

	'882' => {
		message => q||,
		lastUpdated => 
	},

	'cache type' => {
		message => q||,
		lastUpdated => 
	},

	'331' => {
		message => q||,
		lastUpdated => 
	},

	'358' => {
		message => q||,
		lastUpdated => 
	},

	'1050' => {
		message => q||,
		lastUpdated => 
	},

	'560' => {
		message => q||,
		lastUpdated => 
	},

	'737' => {
		message => q||,
		lastUpdated => 
	},

	'84' => {
		message => q||,
		lastUpdated => 
	},

	'437' => {
		message => q|Statistika|,
		lastUpdated => 1178731440
	},

	'glossary title' => {
		message => q||,
		lastUpdated => 
	},

	'451' => {
		message => q|je zahtevano.|,
		lastUpdated => 1178239629
	},

	'changeUrl workflow help' => {
		message => q||,
		lastUpdated => 
	},

	'454' => {
		message => q||,
		lastUpdated => 
	},

	'1021' => {
		message => q||,
		lastUpdated => 
	},

	'135 description' => {
		message => q||,
		lastUpdated => 
	},

	'540 description' => {
		message => q||,
		lastUpdated => 
	},

	'help toc' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageLoop' => {
		message => q||,
		lastUpdated => 
	},

	'978' => {
		message => q||,
		lastUpdated => 
	},

	'728' => {
		message => q||,
		lastUpdated => 
	},

	'float' => {
		message => q||,
		lastUpdated => 
	},

	'purge workflow' => {
		message => q||,
		lastUpdated => 
	},

	'948' => {
		message => q|Odložišče|,
		lastUpdated => 1178731691
	},

	'325' => {
		message => q||,
		lastUpdated => 
	},

	'378' => {
		message => q||,
		lastUpdated => 
	},

	'889' => {
		message => q||,
		lastUpdated => 
	},

	'572' => {
		message => q||,
		lastUpdated => 
	},

	'350' => {
		message => q||,
		lastUpdated => 
	},

	'968' => {
		message => q||,
		lastUpdated => 
	},

	'540' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.lastPageUrl' => {
		message => q||,
		lastUpdated => 
	},

	'58' => {
		message => q||,
		lastUpdated => 
	},

	'431' => {
		message => q||,
		lastUpdated => 
	},

	'527' => {
		message => q|Privzeta izhodiščna stran|,
		lastUpdated => 1178731857
	},

	'861' => {
		message => q||,
		lastUpdated => 
	},

	'manage users in group body' => {
		message => q||,
		lastUpdated => 
	},

	'337' => {
		message => q||,
		lastUpdated => 
	},

	'76' => {
		message => q||,
		lastUpdated => 
	},

	'340' => {
		message => q||,
		lastUpdated => 
	},

	'62' => {
		message => q|shrani|,
		lastUpdated => 1178486501
	},

	'311' => {
		message => q||,
		lastUpdated => 
	},

	'982' => {
		message => q||,
		lastUpdated => 
	},

	'139' => {
		message => q||,
		lastUpdated => 
	},

	'859' => {
		message => q||,
		lastUpdated => 
	},

	'739' => {
		message => q||,
		lastUpdated => 
	},

	'992' => {
		message => q||,
		lastUpdated => 
	},

	'418' => {
		message => q||,
		lastUpdated => 
	},

	'168' => {
		message => q||,
		lastUpdated => 
	},

	'706' => {
		message => q||,
		lastUpdated => 
	},

	'14' => {
		message => q||,
		lastUpdated => 
	},

	'135' => {
		message => q||,
		lastUpdated => 
	},

	'348' => {
		message => q||,
		lastUpdated => 
	},

	'bare insufficient' => {
		message => q||,
		lastUpdated => 
	},

	'145' => {
		message => q||,
		lastUpdated => 
	},

	'preview' => {
		message => q||,
		lastUpdated => 
	},

	'142 description' => {
		message => q||,
		lastUpdated => 
	},

	'993' => {
		message => q||,
		lastUpdated => 
	},

	'364' => {
		message => q||,
		lastUpdated => 
	},

	'653' => {
		message => q||,
		lastUpdated => 
	},

	'950' => {
		message => q||,
		lastUpdated => 
	},

	'cache statistics' => {
		message => q||,
		lastUpdated => 
	},

	'486' => {
		message => q||,
		lastUpdated => 
	},

	'160' => {
		message => q||,
		lastUpdated => 
	},

	'509' => {
		message => q||,
		lastUpdated => 
	},

	'367' => {
		message => q||,
		lastUpdated => 
	},

	'8' => {
		message => q||,
		lastUpdated => 
	},

	'976' => {
		message => q||,
		lastUpdated => 
	},

	'1000' => {
		message => q||,
		lastUpdated => 
	},

	'choose an asset' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.firstPage' => {
		message => q||,
		lastUpdated => 
	},

	'43' => {
		message => q||,
		lastUpdated => 
	},

	'485' => {
		message => q||,
		lastUpdated => 
	},

	'391' => {
		message => q||,
		lastUpdated => 
	},

	'743' => {
		message => q||,
		lastUpdated => 
	},

	'523' => {
		message => q||,
		lastUpdated => 
	},

	'manage users in group title' => {
		message => q||,
		lastUpdated => 
	},

	'943' => {
		message => q||,
		lastUpdated => 
	},

	'unknown user' => {
		message => q||,
		lastUpdated => 
	},

	'460' => {
		message => q||,
		lastUpdated => 
	},

	'119' => {
		message => q||,
		lastUpdated => 
	},

	'453' => {
		message => q||,
		lastUpdated => 
	},

	'324' => {
		message => q||,
		lastUpdated => 
	},

	'244' => {
		message => q||,
		lastUpdated => 
	},

	'351' => {
		message => q||,
		lastUpdated => 
	},

	'fieldtype' => {
		message => q||,
		lastUpdated => 
	},

	'999' => {
		message => q||,
		lastUpdated => 
	},

	'61' => {
		message => q||,
		lastUpdated => 
	},

	'430' => {
		message => q||,
		lastUpdated => 
	},

	'replacements edit body' => {
		message => q||,
		lastUpdated => 
	},

	'379' => {
		message => q||,
		lastUpdated => 
	},

	'admin console template' => {
		message => q||,
		lastUpdated => 
	},

	'1035' => {
		message => q||,
		lastUpdated => 
	},

	'settings help' => {
		message => q|Nastavitve vam omogočajo, da privzete WebGUI vrednosti prilagodite svojim potrebam. Spodaj so razporejene glede na razvrstitev po zavihkih:|,
		lastUpdated => 1178733700
	},

	'452' => {
		message => q|Prosimo, počakajte...|,
		lastUpdated => 1178485362
	},

	'342' => {
		message => q||,
		lastUpdated => 
	},

	'862' => {
		message => q||,
		lastUpdated => 
	},

	'480' => {
		message => q||,
		lastUpdated => 
	},

	'341' => {
		message => q||,
		lastUpdated => 
	},

	'438' => {
		message => q||,
		lastUpdated => 
	},

	'107' => {
		message => q||,
		lastUpdated => 
	},

	'87' => {
		message => q||,
		lastUpdated => 
	},

	'77' => {
		message => q||,
		lastUpdated => 
	},

	'444' => {
		message => q||,
		lastUpdated => 
	},

	'865 description' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.lastPage' => {
		message => q||,
		lastUpdated => 
	},

	'39' => {
		message => q||,
		lastUpdated => 
	},

	'account' => {
		message => q||,
		lastUpdated => 
	},

	'1004 description' => {
		message => q||,
		lastUpdated => 
	},

	'mail return path' => {
		message => q||,
		lastUpdated => 
	},

	'64' => {
		message => q||,
		lastUpdated => 
	},

	'558' => {
		message => q||,
		lastUpdated => 
	},

	'password clear text' => {
		message => q||,
		lastUpdated => 
	},

	'12' => {
		message => q||,
		lastUpdated => 
	},

	'881' => {
		message => q||,
		lastUpdated => 
	},

	'312' => {
		message => q||,
		lastUpdated => 
	},

	'707 description' => {
		message => q||,
		lastUpdated => 
	},

	'45' => {
		message => q||,
		lastUpdated => 
	},

	'507' => {
		message => q||,
		lastUpdated => 
	},

	'405' => {
		message => q||,
		lastUpdated => 
	},

	'1002' => {
		message => q||,
		lastUpdated => 
	},

	'866 description' => {
		message => q||,
		lastUpdated => 
	},

	'1084' => {
		message => q||,
		lastUpdated => 
	},

	'370' => {
		message => q||,
		lastUpdated => 
	},

	'309' => {
		message => q||,
		lastUpdated => 
	},

	'authentication' => {
		message => q||,
		lastUpdated => 
	},

	'734' => {
		message => q||,
		lastUpdated => 
	},

	'1' => {
		message => q||,
		lastUpdated => 
	},

	'144' => {
		message => q||,
		lastUpdated => 
	},

	'965' => {
		message => q||,
		lastUpdated => 
	},

	'824' => {
		message => q||,
		lastUpdated => 
	},

	'661' => {
		message => q||,
		lastUpdated => 
	},

	'857 description' => {
		message => q||,
		lastUpdated => 
	},

	'1075' => {
		message => q||,
		lastUpdated => 
	},

	'381' => {
		message => q||,
		lastUpdated => 
	},

	'581' => {
		message => q||,
		lastUpdated => 
	},

	'418 description' => {
		message => q||,
		lastUpdated => 
	},

	'392' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageCount.isMultiple' => {
		message => q||,
		lastUpdated => 
	},

	'496' => {
		message => q||,
		lastUpdated => 
	},

	'Enable Metadata description' => {
		message => q||,
		lastUpdated => 
	},

	'605' => {
		message => q||,
		lastUpdated => 
	},

	'813' => {
		message => q||,
		lastUpdated => 
	},

	'head.tags' => {
		message => q||,
		lastUpdated => 
	},

	'42' => {
		message => q||,
		lastUpdated => 
	},

	'link in new window' => {
		message => q||,
		lastUpdated => 
	},

	'1005 description' => {
		message => q||,
		lastUpdated => 
	},

	'groups to delete' => {
		message => q||,
		lastUpdated => 
	},

	'1073' => {
		message => q||,
		lastUpdated => 
	},

	'436' => {
		message => q||,
		lastUpdated => 
	},

	'51' => {
		message => q|Geslo|,
		lastUpdated => 1178486358
	},

	'816' => {
		message => q||,
		lastUpdated => 
	},

	'456' => {
		message => q||,
		lastUpdated => 
	},

	'1051 description' => {
		message => q||,
		lastUpdated => 
	},

	'975' => {
		message => q||,
		lastUpdated => 
	},

	'465 description' => {
		message => q||,
		lastUpdated => 
	},

	'link in same window' => {
		message => q||,
		lastUpdated => 
	},

	'493' => {
		message => q||,
		lastUpdated => 
	},

	'trash workflow help' => {
		message => q||,
		lastUpdated => 
	},

	'webgui tips title' => {
		message => q||,
		lastUpdated => 
	},

	'photo' => {
		message => q||,
		lastUpdated => 
	},

	'file' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.nextPage' => {
		message => q||,
		lastUpdated => 
	},

	'445' => {
		message => q||,
		lastUpdated => 
	},

	'1026' => {
		message => q||,
		lastUpdated => 
	},

	'329' => {
		message => q||,
		lastUpdated => 
	},

	'noldaplink' => {
		message => q||,
		lastUpdated => 
	},

	'161' => {
		message => q||,
		lastUpdated => 
	},

	'582' => {
		message => q||,
		lastUpdated => 
	},

	'default version tag workflow help' => {
		message => q||,
		lastUpdated => 
	},

	'125 description' => {
		message => q||,
		lastUpdated => 
	},

	'746' => {
		message => q||,
		lastUpdated => 
	},

	'400' => {
		message => q||,
		lastUpdated => 
	},

	'744' => {
		message => q||,
		lastUpdated => 
	},

	'content filter body' => {
		message => q||,
		lastUpdated => 
	},

	'1011' => {
		message => q||,
		lastUpdated => 
	},

	'556 description' => {
		message => q||,
		lastUpdated => 
	},

	'84 description groupings' => {
		message => q||,
		lastUpdated => 
	},

	'557' => {
		message => q||,
		lastUpdated => 
	},

	'grouping edit title' => {
		message => q||,
		lastUpdated => 
	},

	'369 description' => {
		message => q||,
		lastUpdated => 
	},

	'700' => {
		message => q||,
		lastUpdated => 
	},

	'475' => {
		message => q||,
		lastUpdated => 
	},

	'441' => {
		message => q||,
		lastUpdated => 
	},

	'868' => {
		message => q||,
		lastUpdated => 
	},

	'color' => {
		message => q||,
		lastUpdated => 
	},

	'990' => {
		message => q||,
		lastUpdated => 
	},

	'349' => {
		message => q||,
		lastUpdated => 
	},

	'983' => {
		message => q||,
		lastUpdated => 
	},

	'138' => {
		message => q||,
		lastUpdated => 
	},

	'751' => {
		message => q||,
		lastUpdated => 
	},

	'60' => {
		message => q||,
		lastUpdated => 
	},

	'messaging' => {
		message => q||,
		lastUpdated => 
	},

	'724' => {
		message => q||,
		lastUpdated => 
	},

	'432' => {
		message => q||,
		lastUpdated => 
	},

	'1074' => {
		message => q||,
		lastUpdated => 
	},

	'860' => {
		message => q||,
		lastUpdated => 
	},

	'346' => {
		message => q||,
		lastUpdated => 
	},

	'1075 description' => {
		message => q||,
		lastUpdated => 
	},

	'333' => {
		message => q||,
		lastUpdated => 
	},

	'groups to add' => {
		message => q||,
		lastUpdated => 
	},

	'323' => {
		message => q||,
		lastUpdated => 
	},

	'codearea' => {
		message => q||,
		lastUpdated => 
	},

	'856' => {
		message => q||,
		lastUpdated => 
	},

	'grouping edit body' => {
		message => q||,
		lastUpdated => 
	},

	'69' => {
		message => q||,
		lastUpdated => 
	},

	'glossary body' => {
		message => q||,
		lastUpdated => 
	},

	'removeLabel' => {
		message => q||,
		lastUpdated => 
	},

	'756' => {
		message => q||,
		lastUpdated => 
	},

	'hexadecimal' => {
		message => q||,
		lastUpdated => 
	},

	'446' => {
		message => q||,
		lastUpdated => 
	},

	'559 description' => {
		message => q||,
		lastUpdated => 
	},

	'352' => {
		message => q||,
		lastUpdated => 
	},

	'130 description' => {
		message => q||,
		lastUpdated => 
	},

	'1006' => {
		message => q||,
		lastUpdated => 
	},

	'company' => {
		message => q|Podjetje|,
		lastUpdated => 1178734185
	},

	'126' => {
		message => q|E-naslov podjetja|,
		lastUpdated => 1178734141
	},

	'426' => {
		message => q|Zgodovina prijav|,
		lastUpdated => 1178730643
	},

	'369' => {
		message => q||,
		lastUpdated => 
	},

	'961' => {
		message => q||,
		lastUpdated => 
	},

	'483' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.nextPageText' => {
		message => q||,
		lastUpdated => 
	},

	'button' => {
		message => q||,
		lastUpdated => 
	},

	'810' => {
		message => q|pošlji|,
		lastUpdated => 1178486567
	},

	'372' => {
		message => q||,
		lastUpdated => 
	},

	'574' => {
		message => q||,
		lastUpdated => 
	},

	'170' => {
		message => q||,
		lastUpdated => 
	},

	'default rich editor' => {
		message => q||,
		lastUpdated => 
	},

	'660' => {
		message => q||,
		lastUpdated => 
	},

	'428' => {
		message => q||,
		lastUpdated => 
	},

	'default rich editor description' => {
		message => q||,
		lastUpdated => 
	},

	'977' => {
		message => q||,
		lastUpdated => 
	},

	'99' => {
		message => q||,
		lastUpdated => 
	},

	'526' => {
		message => q||,
		lastUpdated => 
	},

	'72' => {
		message => q||,
		lastUpdated => 
	},

	'566' => {
		message => q||,
		lastUpdated => 
	},

	'806' => {
		message => q||,
		lastUpdated => 
	},

	'869' => {
		message => q||,
		lastUpdated => 
	},

	'help contents' => {
		message => q||,
		lastUpdated => 
	},

	'533' => {
		message => q||,
		lastUpdated => 
	},

	'359' => {
		message => q||,
		lastUpdated => 
	},

	'527 description' => {
		message => q||,
		lastUpdated => 
	},

	'811 description' => {
		message => q||,
		lastUpdated => 
	},

	'1006 description' => {
		message => q||,
		lastUpdated => 
	},

	'108' => {
		message => q||,
		lastUpdated => 
	},

	'1001' => {
		message => q||,
		lastUpdated => 
	},

	'556' => {
		message => q||,
		lastUpdated => 
	},

	'462' => {
		message => q||,
		lastUpdated => 
	},

	'232' => {
		message => q||,
		lastUpdated => 
	},

	'477' => {
		message => q||,
		lastUpdated => 
	},

	'815' => {
		message => q||,
		lastUpdated => 
	},

	'142' => {
		message => q||,
		lastUpdated => 
	},

	'330' => {
		message => q||,
		lastUpdated => 
	},

	'991' => {
		message => q||,
		lastUpdated => 
	},

	'167' => {
		message => q||,
		lastUpdated => 
	},

	'help index' => {
		message => q||,
		lastUpdated => 
	},

	'group' => {
		message => q||,
		lastUpdated => 
	},

	'360' => {
		message => q||,
		lastUpdated => 
	},

	'610' => {
		message => q||,
		lastUpdated => 
	},

	'514' => {
		message => q||,
		lastUpdated => 
	},

	'725' => {
		message => q||,
		lastUpdated => 
	},

	'country' => {
		message => q||,
		lastUpdated => 
	},

	'50 description' => {
		message => q||,
		lastUpdated => 
	},

	'615' => {
		message => q||,
		lastUpdated => 
	},

	'50' => {
		message => q|Uporabniško ime|,
		lastUpdated => 1178486107
	},

	'476' => {
		message => q||,
		lastUpdated => 
	},

	'885 description' => {
		message => q||,
		lastUpdated => 
	},

	'969' => {
		message => q||,
		lastUpdated => 
	},

	'radio' => {
		message => q||,
		lastUpdated => 
	},

	'1076' => {
		message => q||,
		lastUpdated => 
	},

	'510' => {
		message => q||,
		lastUpdated => 
	},

	'1077' => {
		message => q||,
		lastUpdated => 
	},

	'449' => {
		message => q||,
		lastUpdated => 
	},

	'658' => {
		message => q||,
		lastUpdated => 
	},

	'967' => {
		message => q||,
		lastUpdated => 
	},

	'322' => {
		message => q||,
		lastUpdated => 
	},

	'164 description' => {
		message => q||,
		lastUpdated => 
	},

	'353' => {
		message => q||,
		lastUpdated => 
	},

	'575' => {
		message => q||,
		lastUpdated => 
	},

	'863 description' => {
		message => q||,
		lastUpdated => 
	},

	'984' => {
		message => q||,
		lastUpdated => 
	},

	'1039' => {
		message => q||,
		lastUpdated => 
	},

	'1005' => {
		message => q||,
		lastUpdated => 
	},

	'40' => {
		message => q||,
		lastUpdated => 
	},

	'310' => {
		message => q||,
		lastUpdated => 
	},

	'interval' => {
		message => q||,
		lastUpdated => 
	},

	'699' => {
		message => q||,
		lastUpdated => 
	},

	'960' => {
		message => q||,
		lastUpdated => 
	},

	'cancel' => {
		message => q|prekliči|,
		lastUpdated => 1178486598
	},

	'818' => {
		message => q||,
		lastUpdated => 
	},

	'130' => {
		message => q||,
		lastUpdated => 
	},

	'543' => {
		message => q||,
		lastUpdated => 
	},

	'topicName' => {
		message => q||,
		lastUpdated => 
	},

	'941' => {
		message => q||,
		lastUpdated => 
	},

	'manage groups in group body' => {
		message => q||,
		lastUpdated => 
	},

	'354' => {
		message => q||,
		lastUpdated => 
	},

	'461' => {
		message => q||,
		lastUpdated => 
	},

	'583' => {
		message => q||,
		lastUpdated => 
	},

	'debug ip description' => {
		message => q||,
		lastUpdated => 
	},

	'951' => {
		message => q||,
		lastUpdated => 
	},

	'show performance indicators' => {
		message => q||,
		lastUpdated => 
	},

	'85' => {
		message => q||,
		lastUpdated => 
	},

	'809' => {
		message => q||,
		lastUpdated => 
	},

	'332' => {
		message => q||,
		lastUpdated => 
	},

	'9' => {
		message => q||,
		lastUpdated => 
	},

	'425' => {
		message => q|Dejavne seje|,
		lastUpdated => 1178730859
	},

	'packages' => {
		message => q||,
		lastUpdated => 
	},

	'leftovers from settings stored for later' => {
		message => q||,
		lastUpdated => 
	},

	'745' => {
		message => q||,
		lastUpdated => 
	},

	'736' => {
		message => q||,
		lastUpdated => 
	},

	'539' => {
		message => q||,
		lastUpdated => 
	},

	'230 description' => {
		message => q||,
		lastUpdated => 
	},

	'90' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.isLastPage' => {
		message => q||,
		lastUpdated => 
	},

	'565' => {
		message => q||,
		lastUpdated => 
	},

	'620' => {
		message => q||,
		lastUpdated => 
	},

	'977 description' => {
		message => q||,
		lastUpdated => 
	},

	'1004' => {
		message => q||,
		lastUpdated => 
	},

	'891' => {
		message => q||,
		lastUpdated => 
	},

	'1045' => {
		message => q||,
		lastUpdated => 
	},

	'532' => {
		message => q||,
		lastUpdated => 
	},

	'730' => {
		message => q||,
		lastUpdated => 
	},

	'1069' => {
		message => q||,
		lastUpdated => 
	},

	'57' => {
		message => q||,
		lastUpdated => 
	},

	'368' => {
		message => q||,
		lastUpdated => 
	},

	'872' => {
		message => q||,
		lastUpdated => 
	},

	'image manager' => {
		message => q||,
		lastUpdated => 
	},

	'316' => {
		message => q||,
		lastUpdated => 
	},

	'target description' => {
		message => q||,
		lastUpdated => 
	},

	'163' => {
		message => q||,
		lastUpdated => 
	},

	'trash' => {
		message => q|Smeti|,
		lastUpdated => 1178236149
	},

	'manage cache' => {
		message => q|Predpomnilnik|,
		lastUpdated => 1178731152
	},

	'read only' => {
		message => q||,
		lastUpdated => 
	},

	'combobox' => {
		message => q||,
		lastUpdated => 
	},

	'994' => {
		message => q||,
		lastUpdated => 
	},

	'395' => {
		message => q||,
		lastUpdated => 
	},

	'89' => {
		message => q|Skupine|,
		lastUpdated => 1178731526
	},

	'175' => {
		message => q||,
		lastUpdated => 
	},

	'allowed keywords description' => {
		message => q||,
		lastUpdated => 
	},

	'988' => {
		message => q||,
		lastUpdated => 
	},

	'35' => {
		message => q||,
		lastUpdated => 
	},

	'141 description' => {
		message => q||,
		lastUpdated => 
	},

	'347' => {
		message => q||,
		lastUpdated => 
	},

	'665' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.url' => {
		message => q||,
		lastUpdated => 
	},

	'434' => {
		message => q||,
		lastUpdated => 
	},

	'93' => {
		message => q||,
		lastUpdated => 
	},

	'expire groupings' => {
		message => q||,
		lastUpdated => 
	},

	'865' => {
		message => q||,
		lastUpdated => 
	},

	'int slider' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageList' => {
		message => q||,
		lastUpdated => 
	},

	'538 description' => {
		message => q||,
		lastUpdated => 
	},

	'442' => {
		message => q||,
		lastUpdated => 
	},

	'429' => {
		message => q||,
		lastUpdated => 
	},

	'image' => {
		message => q||,
		lastUpdated => 
	},

	'886' => {
		message => q||,
		lastUpdated => 
	},

	'73' => {
		message => q|Prijava.|,
		lastUpdated => 1178485645
	},

	'67' => {
		message => q||,
		lastUpdated => 
	},

	'812' => {
		message => q||,
		lastUpdated => 
	},

	'794' => {
		message => q||,
		lastUpdated => 
	},

	'327' => {
		message => q||,
		lastUpdated => 
	},

	'320' => {
		message => q||,
		lastUpdated => 
	},

	'944' => {
		message => q||,
		lastUpdated => 
	},

	'user' => {
		message => q||,
		lastUpdated => 
	},

	'732' => {
		message => q||,
		lastUpdated => 
	},

	'51 description' => {
		message => q||,
		lastUpdated => 
	},

	'811' => {
		message => q||,
		lastUpdated => 
	},

	'994 description' => {
		message => q||,
		lastUpdated => 
	},

	'957' => {
		message => q||,
		lastUpdated => 
	},

	'622' => {
		message => q||,
		lastUpdated => 
	},

	'361' => {
		message => q||,
		lastUpdated => 
	},

	'465' => {
		message => q||,
		lastUpdated => 
	},

	'contains' => {
		message => q||,
		lastUpdated => 
	},

	'819' => {
		message => q||,
		lastUpdated => 
	},

	'purge workflow help' => {
		message => q||,
		lastUpdated => 
	},

	'970' => {
		message => q||,
		lastUpdated => 
	},

	'link enter alert' => {
		message => q||,
		lastUpdated => 
	},

	'debug ip' => {
		message => q||,
		lastUpdated => 
	},

	'858' => {
		message => q||,
		lastUpdated => 
	},

	'104' => {
		message => q||,
		lastUpdated => 
	},

	'412' => {
		message => q||,
		lastUpdated => 
	},

	'954' => {
		message => q||,
		lastUpdated => 
	},

	'submit' => {
		message => q|Pošlji|,
		lastUpdated => 1178236222
	},

	'314' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.previousPageText' => {
		message => q||,
		lastUpdated => 
	},

	'985' => {
		message => q||,
		lastUpdated => 
	},

	'945 description' => {
		message => q||,
		lastUpdated => 
	},

	'971' => {
		message => q||,
		lastUpdated => 
	},

	'754' => {
		message => q||,
		lastUpdated => 
	},

	'page statistics' => {
		message => q||,
		lastUpdated => 
	},

	'355' => {
		message => q||,
		lastUpdated => 
	},

	'847' => {
		message => q||,
		lastUpdated => 
	},

	'159' => {
		message => q||,
		lastUpdated => 
	},

	'553' => {
		message => q||,
		lastUpdated => 
	},

	'704' => {
		message => q||,
		lastUpdated => 
	},

	'326' => {
		message => q||,
		lastUpdated => 
	},

	'body.content' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageLoop.upTo20' => {
		message => q||,
		lastUpdated => 
	},

	'555' => {
		message => q||,
		lastUpdated => 
	},

	'681' => {
		message => q||,
		lastUpdated => 
	},

	'1017' => {
		message => q||,
		lastUpdated => 
	},

	'37' => {
		message => q||,
		lastUpdated => 
	},

	'335' => {
		message => q||,
		lastUpdated => 
	},

	'1029' => {
		message => q||,
		lastUpdated => 
	},

	'698' => {
		message => q||,
		lastUpdated => 
	},

	'538' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.firstPageUrl' => {
		message => q||,
		lastUpdated => 
	},

	'554' => {
		message => q||,
		lastUpdated => 
	},

	'content' => {
		message => q||,
		lastUpdated => 
	},

	'starts with' => {
		message => q||,
		lastUpdated => 
	},

	'552' => {
		message => q||,
		lastUpdated => 
	},

	'hex slider' => {
		message => q||,
		lastUpdated => 
	},

	'880' => {
		message => q||,
		lastUpdated => 
	},

	'433' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.previousPageUrl' => {
		message => q||,
		lastUpdated => 
	},

	'74' => {
		message => q||,
		lastUpdated => 
	},

	'240' => {
		message => q||,
		lastUpdated => 
	},

	'334' => {
		message => q||,
		lastUpdated => 
	},

	'986' => {
		message => q||,
		lastUpdated => 
	},

	'890' => {
		message => q||,
		lastUpdated => 
	},

	'440' => {
		message => q||,
		lastUpdated => 
	},

	'230' => {
		message => q||,
		lastUpdated => 
	},

	'1008' => {
		message => q||,
		lastUpdated => 
	},

	'1027' => {
		message => q||,
		lastUpdated => 
	},

	'user function style' => {
		message => q||,
		lastUpdated => 
	},

	'871' => {
		message => q||,
		lastUpdated => 
	},

	'misc' => {
		message => q||,
		lastUpdated => 
	},

	'1044' => {
		message => q||,
		lastUpdated => 
	},

	'admin console template description' => {
		message => q||,
		lastUpdated => 
	},

	'1072' => {
		message => q||,
		lastUpdated => 
	},

	'pages' => {
		message => q||,
		lastUpdated => 
	},

	'118 description' => {
		message => q||,
		lastUpdated => 
	},

	'612' => {
		message => q||,
		lastUpdated => 
	},

	'126 description' => {
		message => q||,
		lastUpdated => 
	},

	'991 description' => {
		message => q||,
		lastUpdated => 
	},

	'827' => {
		message => q||,
		lastUpdated => 
	},

	'91' => {
		message => q||,
		lastUpdated => 
	},

	'1086' => {
		message => q||,
		lastUpdated => 
	},

	'701' => {
		message => q||,
		lastUpdated => 
	},

	'820' => {
		message => q||,
		lastUpdated => 
	},

	'174' => {
		message => q||,
		lastUpdated => 
	},

	'481' => {
		message => q||,
		lastUpdated => 
	},

	'867' => {
		message => q||,
		lastUpdated => 
	},

	'422' => {
		message => q||,
		lastUpdated => 
	},

	'817' => {
		message => q||,
		lastUpdated => 
	},

	'563' => {
		message => q||,
		lastUpdated => 
	},

	'trash workflow' => {
		message => q||,
		lastUpdated => 
	},

	'731' => {
		message => q||,
		lastUpdated => 
	},

	'41' => {
		message => q||,
		lastUpdated => 
	},

	'link settings' => {
		message => q||,
		lastUpdated => 
	},

	'668' => {
		message => q||,
		lastUpdated => 
	},

	'52' => {
		message => q|prijavi se|,
		lastUpdated => 1178486424
	},

	'750' => {
		message => q||,
		lastUpdated => 
	},

	'657' => {
		message => q||,
		lastUpdated => 
	},

	'229' => {
		message => q||,
		lastUpdated => 
	},

	'104 description' => {
		message => q||,
		lastUpdated => 
	},

	'866' => {
		message => q||,
		lastUpdated => 
	},

	'768' => {
		message => q||,
		lastUpdated => 
	},

	'68' => {
		message => q||,
		lastUpdated => 
	},

	'315' => {
		message => q||,
		lastUpdated => 
	},

	'400 description' => {
		message => q||,
		lastUpdated => 
	},

	'893' => {
		message => q||,
		lastUpdated => 
	},

	'338' => {
		message => q||,
		lastUpdated => 
	},

	'576' => {
		message => q||,
		lastUpdated => 
	},

	'738' => {
		message => q||,
		lastUpdated => 
	},

	'870' => {
		message => q||,
		lastUpdated => 
	},

	'995 description' => {
		message => q||,
		lastUpdated => 
	},

	'484' => {
		message => q||,
		lastUpdated => 
	},

	'1078' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.text' => {
		message => q||,
		lastUpdated => 
	},

	'ui' => {
		message => q|Uporabniški vmesnik|,
		lastUpdated => 1178733768
	},

	'623' => {
		message => q||,
		lastUpdated => 
	},

	'content filters' => {
		message => q||,
		lastUpdated => 
	},

	'insert a link' => {
		message => q||,
		lastUpdated => 
	},

	'328' => {
		message => q||,
		lastUpdated => 
	},

	'avatar' => {
		message => q||,
		lastUpdated => 
	},

	'1085' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.nextPageUrl' => {
		message => q||,
		lastUpdated => 
	},

	'changeUrl workflow' => {
		message => q||,
		lastUpdated => 
	},

	'363' => {
		message => q||,
		lastUpdated => 
	},

	'583 description' => {
		message => q||,
		lastUpdated => 
	},

	'127 description' => {
		message => q||,
		lastUpdated => 
	},

	'1051' => {
		message => q||,
		lastUpdated => 
	},

	'733' => {
		message => q||,
		lastUpdated => 
	},

	'562' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageList.upTo20' => {
		message => q||,
		lastUpdated => 
	},

	'997' => {
		message => q||,
		lastUpdated => 
	},

	'36' => {
		message => q||,
		lastUpdated => 
	},

	'settings' => {
		message => q|Nastavitve|,
		lastUpdated => 1178733393
	},

	'groups default body' => {
		message => q||,
		lastUpdated => 
	},

	'748' => {
		message => q||,
		lastUpdated => 
	},

	'362' => {
		message => q||,
		lastUpdated => 
	},

	'439' => {
		message => q||,
		lastUpdated => 
	},

	'317' => {
		message => q||,
		lastUpdated => 
	},

	'608' => {
		message => q||,
		lastUpdated => 
	},

	'169' => {
		message => q||,
		lastUpdated => 
	},

	'411' => {
		message => q||,
		lastUpdated => 
	},

	'select slider' => {
		message => q||,
		lastUpdated => 
	},

	'705' => {
		message => q||,
		lastUpdated => 
	},

	'478' => {
		message => q||,
		lastUpdated => 
	},

	'942' => {
		message => q||,
		lastUpdated => 
	},

	'955' => {
		message => q||,
		lastUpdated => 
	},

	'974 description' => {
		message => q||,
		lastUpdated => 
	},

	'85 description' => {
		message => q||,
		lastUpdated => 
	},

	'407' => {
		message => q||,
		lastUpdated => 
	},

	'537' => {
		message => q||,
		lastUpdated => 
	},

	'target' => {
		message => q||,
		lastUpdated => 
	},

	'125' => {
		message => q|Ime podjetja|,
		lastUpdated => 1178734119
	},

	'44' => {
		message => q||,
		lastUpdated => 
	},

	'1007' => {
		message => q||,
		lastUpdated => 
	},

	'667' => {
		message => q||,
		lastUpdated => 
	},

	'998' => {
		message => q||,
		lastUpdated => 
	},

	'95' => {
		message => q||,
		lastUpdated => 
	},

	'user function style description' => {
		message => q||,
		lastUpdated => 
	},

	'313' => {
		message => q||,
		lastUpdated => 
	},

	'webgui' => {
		message => q||,
		lastUpdated => 
	},

	'551' => {
		message => q||,
		lastUpdated => 
	},

	'529' => {
		message => q||,
		lastUpdated => 
	},

	'229 description' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.firstPageText' => {
		message => q||,
		lastUpdated => 
	},

	'753' => {
		message => q||,
		lastUpdated => 
	},

	'343' => {
		message => q||,
		lastUpdated => 
	},

	'504' => {
		message => q||,
		lastUpdated => 
	},

	'url extension description' => {
		message => q||,
		lastUpdated => 
	},

	'697' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.previousPage' => {
		message => q||,
		lastUpdated => 
	},

	'default version tag workflow' => {
		message => q||,
		lastUpdated => 
	},

	'987' => {
		message => q||,
		lastUpdated => 
	},

	'Select State' => {
		message => q||,
		lastUpdated => 
	},

	'857' => {
		message => q||,
		lastUpdated => 
	},

	'1010' => {
		message => q||,
		lastUpdated => 
	},

	'707' => {
		message => q||,
		lastUpdated => 
	},

	'964' => {
		message => q||,
		lastUpdated => 
	},

	'65' => {
		message => q||,
		lastUpdated => 
	},

	'Illegal Warning' => {
		message => q||,
		lastUpdated => 
	},

	'367 description' => {
		message => q||,
		lastUpdated => 
	},

	'1069 description' => {
		message => q||,
		lastUpdated => 
	},

	'81' => {
		message => q||,
		lastUpdated => 
	},

	'824 description' => {
		message => q||,
		lastUpdated => 
	},

	'321' => {
		message => q||,
		lastUpdated => 
	},

	'group email title' => {
		message => q||,
		lastUpdated => 
	},

	'86' => {
		message => q||,
		lastUpdated => 
	},

	'792' => {
		message => q||,
		lastUpdated => 
	},

	'975 description' => {
		message => q||,
		lastUpdated => 
	},

	'823' => {
		message => q||,
		lastUpdated => 
	},

	'profile' => {
		message => q||,
		lastUpdated => 
	},

	'371' => {
		message => q||,
		lastUpdated => 
	},

	'1079' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageNumber' => {
		message => q||,
		lastUpdated => 
	},

	'729' => {
		message => q||,
		lastUpdated => 
	},

	'2' => {
		message => q||,
		lastUpdated => 
	},

	'435' => {
		message => q||,
		lastUpdated => 
	},

	'808' => {
		message => q||,
		lastUpdated => 
	},

	'885' => {
		message => q||,
		lastUpdated => 
	},

	'884' => {
		message => q||,
		lastUpdated => 
	},

	'147' => {
		message => q|Gradniki|,
		lastUpdated => 1178731060
	},

	'339' => {
		message => q||,
		lastUpdated => 
	},

	'1046' => {
		message => q||,
		lastUpdated => 
	},

	'863' => {
		message => q||,
		lastUpdated => 
	},

	'531' => {
		message => q||,
		lastUpdated => 
	},

	'345' => {
		message => q||,
		lastUpdated => 
	},

	'319' => {
		message => q||,
		lastUpdated => 
	},

	'56 description' => {
		message => q||,
		lastUpdated => 
	},

	'1052' => {
		message => q||,
		lastUpdated => 
	},

	'group email body' => {
		message => q||,
		lastUpdated => 
	},

	'735' => {
		message => q||,
		lastUpdated => 
	},

	'404' => {
		message => q||,
		lastUpdated => 
	},

	'tinymce' => {
		message => q||,
		lastUpdated => 
	},

	'613' => {
		message => q||,
		lastUpdated => 
	},

	'1043' => {
		message => q||,
		lastUpdated => 
	},

	'Enable Metadata' => {
		message => q||,
		lastUpdated => 
	},

	'974' => {
		message => q||,
		lastUpdated => 
	},

	'420' => {
		message => q||,
		lastUpdated => 
	},

	'show performance indicators description' => {
		message => q||,
		lastUpdated => 
	},

	'702' => {
		message => q||,
		lastUpdated => 
	},

	'952' => {
		message => q||,
		lastUpdated => 
	},

	'487' => {
		message => q||,
		lastUpdated => 
	},

	'1071' => {
		message => q||,
		lastUpdated => 
	},

	'select' => {
		message => q||,
		lastUpdated => 
	},

	'636' => {
		message => q||,
		lastUpdated => 
	},

	'561' => {
		message => q||,
		lastUpdated => 
	},

	'Enable passive profiling description' => {
		message => q||,
		lastUpdated => 
	},

	'357' => {
		message => q||,
		lastUpdated => 
	},

	'hidden' => {
		message => q||,
		lastUpdated => 
	},

	'63' => {
		message => q||,
		lastUpdated => 
	},

	'455' => {
		message => q||,
		lastUpdated => 
	},

	'1050 description' => {
		message => q||,
		lastUpdated => 
	},

	'80' => {
		message => q||,
		lastUpdated => 
	},

	'336' => {
		message => q||,
		lastUpdated => 
	},

	'mail return path help' => {
		message => q||,
		lastUpdated => 
	},

	'457' => {
		message => q||,
		lastUpdated => 
	},

	'821' => {
		message => q||,
		lastUpdated => 
	},

	'ends with' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.lastPageText' => {
		message => q||,
		lastUpdated => 
	},

	'92' => {
		message => q||,
		lastUpdated => 
	},

	'879' => {
		message => q||,
		lastUpdated => 
	},

	'539 description' => {
		message => q||,
		lastUpdated => 
	},

	'asset locked' => {
		message => q||,
		lastUpdated => 
	},

	'10' => {
		message => q||,
		lastUpdated => 
	},

	'980' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageCount' => {
		message => q||,
		lastUpdated => 
	},

	'864 description' => {
		message => q||,
		lastUpdated => 
	},

	'992 description' => {
		message => q||,
		lastUpdated => 
	},

	'419' => {
		message => q||,
		lastUpdated => 
	},

	'958' => {
		message => q||,
		lastUpdated => 
	},

	'995' => {
		message => q||,
		lastUpdated => 
	},

	'837' => {
		message => q|Mapa, Dodaj/Uredi|,
		lastUpdated => 1178732038
	},

	'149' => {
		message => q|Uporabniki|,
		lastUpdated => 1178731618
	},

	'304' => {
		message => q||,
		lastUpdated => 
	},

	'hidden list' => {
		message => q||,
		lastUpdated => 
	},

	'edit user karma body' => {
		message => q||,
		lastUpdated => 
	},

	'406' => {
		message => q||,
		lastUpdated => 
	},

	'allowed keywords' => {
		message => q||,
		lastUpdated => 
	},

	'482' => {
		message => q||,
		lastUpdated => 
	},

	'949' => {
		message => q||,
		lastUpdated => 
	},

	'Enable passive profiling' => {
		message => q||,
		lastUpdated => 
	},

	'530' => {
		message => q||,
		lastUpdated => 
	},

	'499' => {
		message => q||,
		lastUpdated => 
	},

	'56' => {
		message => q||,
		lastUpdated => 
	},

	'557 description' => {
		message => q||,
		lastUpdated => 
	},

	'66' => {
		message => q|Prijava|,
		lastUpdated => 1178485672
	},

	'show my fields' => {
		message => q||,
		lastUpdated => 
	},

	'54' => {
		message => q||,
		lastUpdated => 
	},

	'1030' => {
		message => q||,
		lastUpdated => 
	},

	'done' => {
		message => q||,
		lastUpdated => 
	},

	'70' => {
		message => q||,
		lastUpdated => 
	},

	'1047' => {
		message => q||,
		lastUpdated => 
	},

	'88' => {
		message => q||,
		lastUpdated => 
	},

	'user profiling' => {
		message => q||,
		lastUpdated => 
	},

	'1009' => {
		message => q||,
		lastUpdated => 
	},

	'groups default title' => {
		message => q||,
		lastUpdated => 
	},

	'141' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageList.upTo10' => {
		message => q||,
		lastUpdated => 
	},

	'403' => {
		message => q||,
		lastUpdated => 
	},

	'883' => {
		message => q||,
		lastUpdated => 
	},

	'clear cache' => {
		message => q|Izprazni predpomnilnik|,
		lastUpdated => 1178731299
	},

	'75' => {
		message => q||,
		lastUpdated => 
	},

	'134' => {
		message => q||,
		lastUpdated => 
	},

	'655' => {
		message => q||,
		lastUpdated => 
	},

	'848' => {
		message => q||,
		lastUpdated => 
	},

	'421' => {
		message => q||,
		lastUpdated => 
	},

	'59' => {
		message => q||,
		lastUpdated => 
	},

	'450' => {
		message => q||,
		lastUpdated => 
	},

	'406 description' => {
		message => q||,
		lastUpdated => 
	},

	'50 setup description' => {
		message => q||,
		lastUpdated => 
	},

	'1070' => {
		message => q||,
		lastUpdated => 
	},

	'703' => {
		message => q||,
		lastUpdated => 
	},

	'864' => {
		message => q||,
		lastUpdated => 
	},

	'1016' => {
		message => q||,
		lastUpdated => 
	},

	'help' => {
		message => q|Pomoč|,
		lastUpdated => 1178236273
	},

	'318' => {
		message => q||,
		lastUpdated => 
	},

	'url extension' => {
		message => q||,
		lastUpdated => 
	},

	'105' => {
		message => q||,
		lastUpdated => 
	},

	'972' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.pageLoop.upTo10' => {
		message => q||,
		lastUpdated => 
	},

	'webgui tips body' => {
		message => q||,
		lastUpdated => 
	},

	'databases' => {
		message => q||,
		lastUpdated => 
	},

	'146' => {
		message => q||,
		lastUpdated => 
	},

	'38' => {
		message => q||,
		lastUpdated => 
	},

	'993 description' => {
		message => q||,
		lastUpdated => 
	},

	'pagination.isFirstPage' => {
		message => q||,
		lastUpdated => 
	},

	'164' => {
		message => q||,
		lastUpdated => 
	},

	'807' => {
		message => q||,
		lastUpdated => 
	},

	'945' => {
		message => q||,
		lastUpdated => 
	},

};

1;
